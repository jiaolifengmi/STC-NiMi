...
if __name__ == '__main__':
    ...
    def calculation():
        ...
        for t_i in range(STC_1.calcium_time_window+1,len(t_neuron)):
            ...
            ##30min 后给另一个刺激
            elif t_i * step < thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + STC_1.calcium_time_window*2 + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) \
                    and t_i * step >= thirty_min + STC_1.calcium_time_window + (stimu_Num_strongLFS) * (repeat_interval_strongLFS):
                time_ = 2
                count = int((t_i * step - STC_1.calcium_time_window - thirty_min - (stimu_Num_strongLFS) * (repeat_interval_strongLFS)) / repeat_interval_weakLFS)  ##0-n次刺激,repeat_interval = 1000ms/1次
                ...
                ### weakLFS  900*1s
                V_s[t_i],V_d_1[t_i],V_d_2[t_i],dend2_Ca_channel[t_i] = voltage.V_update(t_i,V_s[t_i-1],V_d_1[t_i-1],V_d_2[t_i-1])
                synapse_Ca_NMDA_weakLFS[t_i],I_nmda_weakLFS_now = synapse_weakLFS.NMDA_Ca(t_i,synapse_Ca_NMDA_weakLFS[t_i - 1],V_d_2[t_i - 1],g_list_nmda_weakLFS,(np.array(preneuron_spikes_time_weakLFS[int(count)])+ thirty_min  + STC_1.calcium_time_window + repeat_interval * count + (stimu_Num_strongLFS) * (repeat_interval_strongLFS)).tolist())
                I_ampa_weakLFS_now = synapse_weakLFS.I_AMPA(t_i,V_d_2[t_i],g_list_ampa_weakLFS,(np.array(preneuron_spikes_time_weakLFS[int(count)])+thirty_min + STC_1.calcium_time_window + repeat_interval * count + (stimu_Num_strongLFS) * (repeat_interval_strongLFS)).tolist(),z_synapse_1[int(np.ceil((t_i*step) / STC_1.tag_prp_check_window))-1])
                voltage.I_syn_d2 = I_nmda_weakLFS_now + I_ampa_weakLFS_now + I_nmda_strongLFS_now + I_ampa_strongLFS_now
            ...
            if t_i * step>1000 and np.mod(t_i * step - STC_1.calcium_time_window, 1000) == 0.0:
                ...
                f = open("cross-capture7_strongLFS+weakLFS_6/firing rate.txt", "a")
        ...
        f = open("cross-capture7_strongLFS+weakLFS_6/运行时间.txt", "a")
    ...
    resultH5 = h5py.File('cross-capture7_strongLFS+weakLFS_6.hdf5', 'w')
    resultH5.create_dataset(name='V_s', data=(np.array(V_s[0:int((STC_1.calcium_time_window*2 + (stimu_Num_strongLFS) * repeat_interval_strongLFS + thirty_min + stimu_Num_weakLFS * repeat_interval_weakLFS)/step)])).tolist())
    resultH5.create_dataset(name='V_d_1', data=(np.array(V_d_1[0:int((STC_1.calcium_time_window*2 + (stimu_Num_strongLFS) * repeat_interval_strongLFS + thirty_min + stimu_Num_weakLFS * repeat_interval_weakLFS)/step)])).tolist())
    resultH5.create_dataset(name='V_d_2', data=(np.array(V_d_2[0:int((STC_1.calcium_time_window*2 + (stimu_Num_strongLFS) * repeat_interval_strongLFS + thirty_min + stimu_Num_weakLFS * repeat_interval_weakLFS)/step)])).tolist())
    resultH5.create_dataset(name='soma_Ca_channel', data=(np.array(soma_Ca_channel[0:int((STC_1.calcium_time_window*2 + (stimu_Num_strongLFS) * repeat_interval_strongLFS + thirty_min + stimu_Num_weakLFS * repeat_interval_weakLFS)/step)])).tolist())
    resultH5.create_dataset(name='dend_Ca_channel', data=(np.array(dend2_Ca_channel[0:int((STC_1.calcium_time_window*2 + (stimu_Num_strongLFS) * repeat_interval_strongLFS + thirty_min + stimu_Num_weakLFS * repeat_interval_weakLFS)/step)])).tolist())
    resultH5.create_dataset(name='synapse_Ca_NMDA_strongLFS', data=(np.array(synapse_Ca_NMDA_strongLFS[0:int((STC_1.calcium_time_window*2 + (stimu_Num_strongLFS) * repeat_interval_strongLFS + thirty_min + stimu_Num_weakLFS * repeat_interval_weakLFS)/step)])).tolist())
    resultH5.create_dataset(name='synapse_Ca_NMDA_weakLFS', data=(np.array(synapse_Ca_NMDA_weakLFS[0:int((STC_1.calcium_time_window*2 + (stimu_Num_strongLFS) * repeat_interval_strongLFS + thirty_min + stimu_Num_weakLFS * repeat_interval_weakLFS)/step)])).tolist())
    ...
    # plt.savefig('cross-capture7_strongLFS+weakLFS_6/1-1_prp1000.jpg',bbox_inches = 'tight')
    #
    #
    # plt.close() # # plt.show()
    #
    ...
    # plt.title(" synaptic weight changes")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture7_strongLFS+weakLFS_6/1-1_1_prp1000.jpg',bbox_inches = 'tight')
    #
    # plt.close()
    #
    # fig = plt.figure(figsize=(6, 5))
    ...
    # plt.title(" synaptic weight changes")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture7_strongLFS+weakLFS_6/1-1_2_prp100.jpg',bbox_inches = 'tight')
    #
    # plt.close()
    #
    # fig = plt.figure(figsize=(6, 5))
    ...
    # plt.title(" synaptic weight changes")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture7_strongLFS+weakLFS_6/1-1_3_prp10.jpg',bbox_inches = 'tight')
    #
    #
    # plt.close() # plt.show()
    #
    ...
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture7_strongLFS+weakLFS_6/1-3_prp100.jpg',bbox_inches = 'tight')
    #
    # plt.close()
    ...
    #
    # for i in range(STC_1.calcium_time_window + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + thirty_min + stimu_Num_weakLFS * repeat_interval_weakLFS + repeat_interval):
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture7_strongLFS+weakLFS_6/1-Ca_average_strongLFS_1.jpg',bbox_inches = 'tight')
    #
    #
    # plt.close() # plt.show()
    #
    ...
    # lns2 = ax1.plot(t_average[0:1500] - STC_1.calcium_time_window, synapse_Ca_NMDA_weakLFS_average[0 + repeat_interval_strongLFS:1500 + repeat_interval_strongLFS],label='  synapse Ca NMDA-weak LFS', color="b")
    ...
    # lns2 = ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,synapse_Ca_NMDA_weakLFS[0 + int(repeat_interval_strongLFS*600 / step):int(1500 / step) + int(repeat_interval_strongLFS*600 / step)],label=' synapse Ca NMDA-weak LFS', color="b")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture7_strongLFS+weakLFS_6/1-Ca_average_strongLFS_2.jpg',bbox_inches = 'tight')
    #
    #
    # plt.close() # plt.show()
    #
    ...
    # lns2 = ax1.plot(t_average[0:1500] - STC_1.calcium_time_window, synapse_Ca_NMDA_weakLFS_average[0 + repeat_interval_strongLFS*800:1500 + repeat_interval_strongLFS*800],label='  synapse Ca NMDA-weak LFS', color="b")
    ...
    # lns2 = ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,synapse_Ca_NMDA_weakLFS[0 + int(repeat_interval_strongLFS*800 / step):int(1500 / step) + int(repeat_interval_strongLFS*800 / step)],label=' synapse Ca NMDA-weak LFS', color="b")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture7_strongLFS+weakLFS_6/1-Ca_average_strongLFS_3.jpg',bbox_inches = 'tight')
    #
    #
    # plt.close() # plt.show()
    #
    ...
    #                 synapse_Ca_NMDA_weakLFS_average[thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS):int(thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + 1500)],
    ...
    # lns2 = ax1.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     synapse_Ca_NMDA_weakLFS[int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS)) / step):int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS)) / step) + int(1500 / step)],
    #     label='  synapse Ca NMDA-weak LFS', color="b")
    # lns3 = ax2.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture7_strongLFS+weakLFS_6/1-Ca_average_weakLFS_1.jpg',bbox_inches = 'tight')
    #
    # plt.close()
    #
    # fig = plt.figure(figsize=(5.8, 4.0))
    ...
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_average[thirty_min:int(thirty_min + 1500)] - thirty_min - 500,
    #                 synapse_Ca_NMDA_strongLFS_average[thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + repeat_interval_weakLFS*600:int(thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS)  + repeat_interval_weakLFS*600 + 1500)],
    #                 label=' synapse Ca NMDA-strong LFS', color="r")
    # lns2 = ax1.plot(t_average[thirty_min:int(thirty_min + 1500)] - thirty_min - 500,
    #                 synapse_Ca_NMDA_weakLFS_average[thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + repeat_interval_weakLFS*600:int(thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + repeat_interval_weakLFS*600 + 1500)],
    #                 label='  synapse Ca NMDA-weak LFS', color="b")
    # lns3 = ax2.plot(t_average[thirty_min:int(thirty_min + 1500)] - thirty_min - 500,
    #                 dend2_Ca_channel_average[thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + repeat_interval_weakLFS*600:int(thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + repeat_interval_weakLFS*600 + 1500)], '--',
    #                 label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca NMDA (μmol/L)", color="black")
    ...
    # ax2 = ax1.twinx()
    # for i in range(len(V_s[int(thirty_min / step):int((thirty_min + 1500) / step)]) - 1):
    #     if i > 0 and V_s[i + int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + repeat_interval_weakLFS*600) / step)] >= V_s[i + int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + repeat_interval_weakLFS*600) / step) - 1] and V_s[
    #         i + int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + repeat_interval_weakLFS*600) / step)] >= V_s[i + int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + repeat_interval_weakLFS*600) / step) + 1] and V_s[
    #         i + int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + repeat_interval_weakLFS*600) / step)] >= -40:
    #         plt.axvline(x=(i * step - STC_1.calcium_time_window), ls='--', c='y', linewidth=1.0)
    # lns1 = ax1.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     synapse_Ca_NMDA_strongLFS[int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + repeat_interval_weakLFS*600) / step):int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + repeat_interval_weakLFS*600) / step) + int(1500 / step)],
    #     label=' synapse Ca NMDA-strong LFS', color="r")
    # lns2 = ax1.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     synapse_Ca_NMDA_weakLFS[int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + repeat_interval_weakLFS*600) / step):int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + repeat_interval_weakLFS*600) / step) + int(1500 / step)],
    #     label='  synapse Ca NMDA-weak LFS', color="b")
    # lns3 = ax2.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     dend2_Ca_channel[int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + repeat_interval_weakLFS*600) / step):int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + repeat_interval_weakLFS*600) / step) + int(1500 / step)], '--',
    #     label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca_NMDA (μmol/L)", color="black")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture7_strongLFS+weakLFS_6/1-Ca_average_weakLFS_600.jpg',bbox_inches = 'tight')
    #
    # plt.close()
    #
    ...
    # plt.savefig('cross-capture7_strongLFS+weakLFS_6/1-8.jpg',bbox_inches = 'tight')
    #
    # plt.close()
    #
    ...
    # plt.legend(loc = 1)
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture7_strongLFS+weakLFS_6/1-9.jpg',bbox_inches = 'tight')
    #
    # plt.close()
    #
    # fig = plt.figure(figsize=(5, 5))
    ...
    # plt.legend(loc = 1)
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture7_strongLFS+weakLFS_6/1-9_1.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(5, 5))
    # ax1 = fig.add_subplot(212)
    ...
    # plt.legend(loc = 1)
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture7_strongLFS+weakLFS_6/1-9_600.jpg',bbox_inches = 'tight')
    #
    # plt.close()
    #
    ...
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture7_strongLFS+weakLFS_6/1-15.jpg',bbox_inches = 'tight')
    #
    # plt.close()
    #
    # fig = plt.figure(figsize=(6, 4.5))
    ...
    #                 label=' synapse Ca NMDA-strong LFS', color="r")
    # lns2 = ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,
    #                 synapse_Ca_NMDA_weakLFS[0 + int((repeat_interval_strongLFS)/step):int(1500 / step) + int((repeat_interval_strongLFS)/step)],
    #                 label='  synapse Ca NMDA-weak LFS', color="b")
    ...
    # plt.title("changes in calcium concentration(strong LFS)")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture7_strongLFS+weakLFS_6/1-15_1.jpg',bbox_inches = 'tight')
    #
    # plt.close()
    #
    # fig = plt.figure(figsize=(6, 4.5))
    ...
    #                 label=' synapse Ca NMDA-strong LFS', color="r")
    # lns2 = ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,
    #                 synapse_Ca_NMDA_weakLFS[0 + int((repeat_interval_strongLFS*600)/step):int(1500 / step) + int((repeat_interval_strongLFS*600)/step)],
    #                 label='  synapse Ca NMDA-weak LFS', color="b")
    # lns3 = ax2.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,
    ...
    # plt.title("changes in calcium concentration(strong LFS)")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture7_strongLFS+weakLFS_6/1-15_2.jpg',bbox_inches = 'tight')
    #
    # plt.close()
    #
    #
    ...
    # plt.title("changes in calcium concentration")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture7_strongLFS+weakLFS_6/1-17.jpg',bbox_inches = 'tight')
    #
    # plt.close()
    #
    # # fig = plt.figure(figsize=(10, 5))
    ...
    # # plt.savefig('cross-capture7_strongLFS+weakLFS_6/1-18.jpg')
    # #
    #
    #
    # fig = plt.figure(figsize=(10, 5))
    ...
    # bax.set_xlabel("time (min)")
    # bax.set_title("postneuron firing rate")
    # plt.savefig('cross-capture7_strongLFS+weakLFS_6/1-19.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    #
    # # fig = plt.figure(figsize=(10, 5))
    ...
    # # bax.legend(loc=1)
    # # plt.title("changes in calcium concentration")
    # # plt.savefig('cross-capture7_strongLFS+weakLFS_6/1_7_1_1.jpg',bbox_inches = 'tight')
    # # plt.close() # plt.show()