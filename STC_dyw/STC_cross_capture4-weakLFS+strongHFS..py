...
if __name__ == '__main__':
    ...
    def calculation():
        ...
        for t_i in range(STC_1.calcium_time_window+1,len(t_neuron)):
            num_strong = int((t_i*step-STC_1.calcium_time_window - thirty_min - (stimu_Num_weakLFS) * (repeat_interval_weakLFS))/repeat_interval_strongHFS)
            if t_i * step < (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + STC_1.calcium_time_window*2:
                ...
                I_nmda_weakLFS_pre = I_nmda_weakLFS_now
                    ...
                    and (t_i * step - num_strong * ten_min - STC_1.calcium_time_window - thirty_min - stimu_Num_weakLFS * repeat_interval_weakLFS) < repeat_interval * 2\
                    and t_i * step >= STC_1.calcium_time_window + stimu_Num_weakLFS * repeat_interval_weakLFS + thirty_min:  ##30min 后给另一个刺激
                time_ = 2
                count = int((t_i * step - STC_1.calcium_time_window - thirty_min - stimu_Num_weakLFS * repeat_interval_weakLFS) / repeat_interval_strongHFS)  ##0-n次刺激,repeat_interval_strongHFS = 10min/1次
                ## strongHFS
                V_s[t_i],V_d_1[t_i],V_d_2[t_i],dend2_Ca_channel[t_i] = voltage.V_update(t_i,V_s[t_i-1],V_d_1[t_i-1],V_d_2[t_i-1])
                synapse_Ca_NMDA_strongHFS[t_i],I_nmda_strongHFS_now = synapse_strongHFS.NMDA_Ca(t_i,synapse_Ca_NMDA_strongHFS[t_i - 1],V_d_2[t_i - 1],g_list_nmda_strongHFS,(np.array(preneuron_spikes_time_strongHFS[count]) + ten_min * count + thirty_min +STC_1.calcium_time_window + (stimu_Num_weakLFS) * (repeat_interval_weakLFS)).tolist())
                I_ampa_strongHFS_now = synapse_strongHFS.I_AMPA(t_i,V_d_2[t_i],g_list_ampa_strongHFS,(np.array(preneuron_spikes_time_strongHFS[int(count)]) + ten_min * count + thirty_min + STC_1.calcium_time_window + (stimu_Num_weakLFS) * (repeat_interval_weakLFS)).tolist(),z_synapse_2[int(np.ceil((t_i*step) / STC_1.tag_prp_check_window))-1])
                voltage.I_syn_d2 = I_nmda_weakLFS_now + I_ampa_weakLFS_now + I_nmda_strongHFS_now + I_ampa_strongHFS_now
            ...
            if t_i * step>1000 and np.mod(t_i * step - STC_1.calcium_time_window, 1000) == 0.0:
                ...
                f = open("cross-capture4_weakLFS+strongHFS_4/firing rate.txt", "a")
        ...
        f = open("cross-capture4_weakLFS+strongHFS_4/运行时间.txt", "a")
    ...
    resultH5 = h5py.File('cross-capture4_weakLFS+strongHFS_4.hdf5', 'w')
    resultH5.create_dataset(name='V_s', data=(np.array(V_s[0:int((STC_1.calcium_time_window*2 + stimu_Num_weakLFS * repeat_interval_weakLFS + thirty_min + (stimu_Num_strongHFS -1) * repeat_interval_strongHFS + time_spike_train_one_strongHFS)/step)])).tolist())
    resultH5.create_dataset(name='V_d_1', data=(np.array(V_d_1[0:int((STC_1.calcium_time_window*2 + stimu_Num_weakLFS * repeat_interval_weakLFS + thirty_min + (stimu_Num_strongHFS -1) * repeat_interval_strongHFS + time_spike_train_one_strongHFS)/step)])).tolist())
    resultH5.create_dataset(name='V_d_2', data=(np.array(V_d_2[0:int((STC_1.calcium_time_window*2 + stimu_Num_weakLFS * repeat_interval_weakLFS + thirty_min + (stimu_Num_strongHFS -1) * repeat_interval_strongHFS + time_spike_train_one_strongHFS)/step)])).tolist())
    resultH5.create_dataset(name='soma_Ca_channel', data=(np.array(soma_Ca_channel[0:int((STC_1.calcium_time_window*2 + stimu_Num_weakLFS * repeat_interval_weakLFS + thirty_min + (stimu_Num_strongHFS -1) * repeat_interval_strongHFS + time_spike_train_one_strongHFS)/step)])).tolist())
    resultH5.create_dataset(name='dend_Ca_channel', data=(np.array(dend2_Ca_channel[0:int((STC_1.calcium_time_window*2 + stimu_Num_weakLFS * repeat_interval_weakLFS + thirty_min + (stimu_Num_strongHFS -1) * repeat_interval_strongHFS + time_spike_train_one_strongHFS)/step)])).tolist())
    resultH5.create_dataset(name='synapse_Ca_NMDA_strongHFS', data=(np.array(synapse_Ca_NMDA_strongHFS[0:int((STC_1.calcium_time_window*2 + stimu_Num_weakLFS * repeat_interval_weakLFS + thirty_min + (stimu_Num_strongHFS -1) * repeat_interval_strongHFS + time_spike_train_one_strongHFS)/step)])).tolist())
    resultH5.create_dataset(name='synapse_Ca_NMDA_weakLFS', data=(np.array(synapse_Ca_NMDA_weakLFS[0:int((STC_1.calcium_time_window*2 + stimu_Num_weakLFS * repeat_interval_weakLFS + thirty_min + (stimu_Num_strongHFS -1) * repeat_interval_strongHFS + time_spike_train_one_strongHFS)/step)])).tolist())
    ...
    # ax4 = ax3.twinx()
    # lns66 = ax3.plot(tag_prp_time / time_scale_tag_prp, z_synapse_2, label='z synapse(strong HFS stimuli)',color='black')
    ...
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-1_prp1000.jpg',bbox_inches = 'tight')
    # plt.close()
    # # # plt.show()
    #
    # # Visualization
    ...
    # plt.title(" synaptic weight changes")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-1_1_prp1000.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(6, 5))
    # ax3 = fig.add_subplot(211)
    ...
    # plt.title(" synaptic weight changes")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-1_2_prp100.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(6, 5))
    # ax3 = fig.add_subplot(211)
    ...
    # plt.title(" synaptic weight changes")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-1_3_prp10.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    # fig = plt.figure(figsize=(6, 5))
    ...
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-3_prp100.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # # plt.show()
    #
    ...
    # # tag_prp_plt_1 = np.zeros(time_neuron)
    # # tag_prp_plt_2 = np.zeros(time_neuron)
    # # for i in range(len(tag_flag_synapse_1)):
    # #     if tag_flag_synapse_1[i] == 1 or tag_flag_synapse_1[i] == -1:
    # #         tag_prp_plt_1[int(i * STC_1.tag_prp_check_window): int((i + 1) * STC_1.tag_prp_check_window)] = tag_flag_synapse_1[i]
    # # time_scale = 60 * 1000  ## min <-> ms
    # # plt.plot(t_tag_prp, tag_prp_plt_1)
    # # plt.xlim(STC_1.calcium_time_window, STC_1.calcium_time_window + 1000)
    # # plt.xlabel("Time (ms)")
    # # plt.ylabel('tag_flag')
    # # plt.title("synapse1 tag flag: low frequency")
    # # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-4.jpg')
    # #
    # # fig = plt.figure(figsize=(10.5, 7.5))
    # # plt.subplot(212)
    # # plt.plot(t_tag_prp / time_scale, tag_prp_plt_1)
    # # plt.xlabel("Time (min)")
    # # plt.ylabel('tag_flag')
    # # plt.xlim(0, 1)
    # # plt.title("synapse1 tag flag: low frequency(1min)")
    # # plt.tight_layout(0.1)
    # # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-5.jpg')
    # # # plt.show()
    # #
    # # fig = plt.figure(figsize=(10.5, 7.5))
    # # plt.subplot(211)
    # # for i in range(len(tag_flag_synapse_2)):
    # #     if tag_flag_synapse_2[i] == 1 or tag_flag_synapse_2[i] == -1:
    # #         tag_prp_plt_2[int(i * STC_1.tag_prp_check_window): int((i + 1) * STC_1.tag_prp_check_window)] = tag_flag_synapse_2[i]
    # # time_scale = 60 * 1000  ## min <-> ms
    # # plt.plot(t_tag_prp, tag_prp_plt_2)
    # # plt.xlim(sixty_min + STC_1.calcium_time_window, STC_1.calcium_time_window + sixty_min + 1000)
    # # plt.xlabel("Time (ms)")
    # # plt.ylabel('tag_flag')
    # # plt.title("synapse2 tag flag: high frequency")
    # # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-6.jpg')
    # #
    # # fig = plt.figure(figsize=(10.5, 7.5))
    # # plt.subplot(211)
    # # time_scale = 60 * 1000  ## min <-> ms
    # # plt.plot(t_tag_prp - STC_1.calcium_time_window, tag_prp_plt_1)
    # # plt.xlim(0, 1000)
    # # plt.xlabel("Time (ms)")
    # # plt.ylabel('tag_flag')
    # # plt.title("synapse1 tag_flag: low frequency")
    # # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-4_1.jpg')
    # #
    # # fig = plt.figure(figsize=(10.5, 7.5))
    # # plt.subplot(211)
    # # time_scale = 60 * 1000  ## min <-> ms
    # # plt.plot(t_tag_prp - STC_1.calcium_time_window - sixty_min, tag_prp_plt_2)
    # # plt.xlim(0, 1000)
    # # plt.xlabel("Time (ms)")
    # # plt.ylabel('tag_flag')
    # # plt.title("synapse2 tag flag: high frequency")
    # # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-6_1.jpg')
    # #
    # # fig = plt.figure(figsize=(10.5, 7.5))
    # # plt.subplot(212)
    # # time_scale = 60 * 1000  ## min <-> ms
    # # plt.plot((t_tag_prp[sixty_min:sixty_min + 60 * 1000] - sixty_min) / time_scale,
    # #          tag_prp_plt_2[sixty_min:sixty_min + 60 * 1000])
    # # plt.xlim(0, 1)
    # # plt.xlabel("Time (min)")
    # # plt.ylabel('tag_flag')
    # # plt.title("synapse2 tag flag: high frequency(1min)")
    # # plt.tight_layout(0.1)
    # # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-7_1.jpg')
    #
    ...
    # # plt.tight_layout(0.1)
    # # plt.savefig('cross-capture4/1-7.jpg')
    # # # # plt.show()
    #
    # step = 0.05
    ...
    #
    # for i in range(STC_1.calcium_time_window + stimu_Num_weakLFS * repeat_interval_weakLFS + thirty_min + (stimu_Num_strongHFS-1) * repeat_interval_strongHFS + time_spike_train_one_strongHFS + repeat_interval):
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-Ca_average_weakLFS_1.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    # fig = plt.figure(figsize=(5.8, 4.0))
    # ax1 = fig.add_subplot(212)
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_average[0:1500] - STC_1.calcium_time_window, synapse_Ca_NMDA_strongHFS_average[0 + repeat_interval_weakLFS:1500 + repeat_interval_weakLFS],
    #                 label=' synapse Ca NMDA-strong HFS stimuli ', color="r")
    ...
    #     if i >0 and V_d_2[i] >= V_d_2[i - 1]  and V_d_2[i] >= V_d_2[i+1] and V_d_2[i] >= -40:
    #         plt.axvline(x = (i*step-STC_1.calcium_time_window),ls = '--',c = 'y',linewidth = 1.0)
    # lns1 = ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,synapse_Ca_NMDA_strongHFS[0 + int(repeat_interval_weakLFS / step):int(1500 / step) + int(repeat_interval_weakLFS / step)],label=' synapse Ca NMDA-strong HFS stimuli ', color="r")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-Ca_average_weakLFS_2.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    # fig = plt.figure(figsize=(5.8, 4.0))
    # ax1 = fig.add_subplot(212)
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_average[0:1500] - STC_1.calcium_time_window, synapse_Ca_NMDA_strongHFS_average[0 + repeat_interval_weakLFS * 100:1500 + repeat_interval_weakLFS * 100],
    #                 label=' synapse Ca NMDA-strong HFS stimuli ', color="r")
    ...
    #     if i >0 and V_d_2[i] >= V_d_2[i - 1]  and V_d_2[i] >= V_d_2[i+1] and V_d_2[i] >= -40:
    #         plt.axvline(x = (i*step-STC_1.calcium_time_window),ls = '--',c = 'y',linewidth = 1.0)
    # lns1 = ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,synapse_Ca_NMDA_strongHFS[0 + int(repeat_interval_weakLFS * 100 / step):int(1500 / step) + int(repeat_interval_weakLFS * 100 / step)],label=' synapse Ca NMDA-strong HFS stimuli ', color="r")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-Ca_average_weakLFS_100.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    # fig = plt.figure(figsize=(5.8, 4.0))
    # ax1 = fig.add_subplot(212)
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_average[0:1500] - STC_1.calcium_time_window, synapse_Ca_NMDA_strongHFS_average[0 + repeat_interval_weakLFS * 200:1500 + repeat_interval_weakLFS * 200],
    #                 label=' synapse Ca NMDA-strong HFS stimuli ', color="r")
    ...
    #     if i >0 and V_d_2[i] >= V_d_2[i - 1]  and V_d_2[i] >= V_d_2[i+1] and V_d_2[i] >= -40:
    #         plt.axvline(x = (i*step-STC_1.calcium_time_window),ls = '--',c = 'y',linewidth = 1.0)
    # lns1 = ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,synapse_Ca_NMDA_strongHFS[0 + int(repeat_interval_weakLFS * 200 / step):int(1500 / step) + int(repeat_interval_weakLFS * 200 / step)],label=' synapse Ca NMDA-strong HFS stimuli ', color="r")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-Ca_average_weakLFS_200.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    # fig = plt.figure(figsize=(5.8, 4.0))
    # ax1 = fig.add_subplot(212)
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_average[0:1500] - STC_1.calcium_time_window, synapse_Ca_NMDA_strongHFS_average[0 + repeat_interval_weakLFS * 600:1500 + repeat_interval_weakLFS * 600],
    #                 label=' synapse Ca NMDA-strong HFS stimuli ', color="r")
    ...
    #     if i >0 and V_d_2[i] >= V_d_2[i - 1]  and V_d_2[i] >= V_d_2[i+1] and V_d_2[i] >= -40:
    #         plt.axvline(x = (i*step-STC_1.calcium_time_window),ls = '--',c = 'y',linewidth = 1.0)
    # lns1 = ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,synapse_Ca_NMDA_strongHFS[0 + int(repeat_interval_weakLFS * 600 / step):int(1500 / step) + int(repeat_interval_weakLFS * 600 / step)],label=' synapse Ca NMDA-strong HFS stimuli ', color="r")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-Ca_average_weakLFS_600.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    #
    ...
    #                 synapse_Ca_NMDA_strongHFS_average[thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS):int(thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + 1500)],
    #                 label=' synapse Ca NMDA-strong HFS stimuli ', color="r")
    ...
    #     synapse_Ca_NMDA_strongHFS[int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS)) / step):int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS)) / step) + int(1500 / step)],
    #     label=' synapse Ca NMDA-strong HFS stimuli', color="r")
    # lns2 = ax1.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-Ca_average_strongHFS_1.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    # fig = plt.figure(figsize=(5.8, 4.0))
    ...
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_average[thirty_min:int(thirty_min + 1500)] - thirty_min - 500,
    #                 synapse_Ca_NMDA_strongHFS_average[thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS)+ten_min:int(thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) +ten_min + 1500)],
    #                 label=' synapse Ca NMDA-strong HFS stimuli', color="r")
    # lns2 = ax1.plot(t_average[thirty_min:int(thirty_min + 1500)] - thirty_min - 500,
    #                 synapse_Ca_NMDA_weakLFS_average[thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min:int(thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min + 1500)],
    #                 label='  synapse Ca NMDA-weak LFS stimuli', color="b")
    # lns3 = ax2.plot(t_average[thirty_min:int(thirty_min + 1500)] - thirty_min - 500,
    #                 dend2_Ca_channel_average[thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min:int(thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min + 1500)], '--',
    #                 label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca NMDA (μmol/L)", color="black")
    ...
    # ax1 = fig.add_subplot(211)
    # ax2 = ax1.twinx()
    # for i in range(len(V_s[int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min) / step):int(((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min) + 1500) / step)]) - 1):
    #     if i > 0 and V_s[i + int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min) / step)] >= V_s[i + int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min) / step) - 1] and V_s[
    #         i + int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min) / step)] >= V_s[i + int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min) / step) + 1] and V_s[
    #         i + int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min) / step)] >= -40:
    #         plt.axvline(x=(i * step - STC_1.calcium_time_window), ls='--', c='y', linewidth=1.0)
    # lns1 = ax1.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     synapse_Ca_NMDA_strongHFS[int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min) / step):int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min) / step) + int(1500 / step)],
    #     label=' synapse Ca NMDA-strong HFS stimuli', color="r")
    # lns2 = ax1.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     synapse_Ca_NMDA_weakLFS[int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min) / step):int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min) / step) + int(1500 / step)],
    #     label='  synapse Ca NMDA-weak LFS stimuli', color="b")
    # lns3 = ax2.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     dend2_Ca_channel[int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min) / step):int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min) / step) + int(1500 / step)], '--',
    #     label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca_NMDA (μmol/L)", color="black")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-Ca_average_strongHFS_2.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    # fig = plt.figure(figsize=(5.8, 4.0))
    ...
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_average[thirty_min:int(thirty_min + 1500)] - thirty_min - 500,
    #                 synapse_Ca_NMDA_strongHFS_average[thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS)+ten_min*2:int(thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) +ten_min*2 + 1500)],
    #                 label=' synapse Ca NMDA-strong HFS stimuli', color="r")
    # lns2 = ax1.plot(t_average[thirty_min:int(thirty_min + 1500)] - thirty_min - 500,
    #                 synapse_Ca_NMDA_weakLFS_average[thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min*2:int(thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min*2 + 1500)],
    #                 label='  synapse Ca NMDA-weak LFS stimuli', color="b")
    # lns3 = ax2.plot(t_average[thirty_min:int(thirty_min + 1500)] - thirty_min - 500,
    #                 dend2_Ca_channel_average[thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min*2:int(thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min*2 + 1500)], '--',
    #                 label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca NMDA (μmol/L)", color="black")
    ...
    # ax1 = fig.add_subplot(211)
    # ax2 = ax1.twinx()
    # for i in range(len(V_s[int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min*2) / step):int(((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min*2) + 1500) / step)]) - 1):
    #     if i > 0 and V_s[i + int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min*2) / step)] >= V_s[i + int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min*2) / step) - 1] and V_s[
    #         i + int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min*2) / step)] >= V_s[i + int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min*2) / step) + 1] and V_s[
    #         i + int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min*2) / step)] >= -40:
    #         plt.axvline(x=(i * step - STC_1.calcium_time_window), ls='--', c='y', linewidth=1.0)
    # lns1 = ax1.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     synapse_Ca_NMDA_strongHFS[int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min*2) / step):int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min*2) / step) + int(1500 / step)],
    #     label=' synapse Ca NMDA-strong HFS stimuli', color="r")
    # lns2 = ax1.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     synapse_Ca_NMDA_weakLFS[int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min*2) / step):int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min*2) / step) + int(1500 / step)],
    #     label='  synapse Ca NMDA-weak LFS stimuli', color="b")
    # lns3 = ax2.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     dend2_Ca_channel[int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min*2) / step):int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + ten_min*2) / step) + int(1500 / step)], '--',
    #     label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca_NMDA (μmol/L)", color="black")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-Ca_average_strongHFS_3.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    ...
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-8.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    ...
    # plt.legend(loc = 1)
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-9.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(5, 5))
    # ax1 = fig.add_subplot(212)
    ...
    # plt.legend(loc = 1)
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-9_1.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(5, 5))
    # ax1 = fig.add_subplot(212)
    ...
    # plt.legend(loc = 1)
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-9_100.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(5, 5))
    # ax1 = fig.add_subplot(212)
    ...
    # plt.legend(loc = 1)
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-9_200.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(5, 5))
    # ax1 = fig.add_subplot(212)
    ...
    # plt.legend(loc = 1)
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-9_600.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(5, 5))
    # ax1 = fig.add_subplot(212)
    ...
    # plt.legend(loc = 1)
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-9_800.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    ...
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-15.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(6, 4.5))
    # ax1 = fig.add_subplot(212)
    ...
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-16_1.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    # fig = plt.figure(figsize=(6, 4.5))
    ...
    # plt.title("changes in calcium concentration(strong HFS)")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-16_2.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    # fig = plt.figure(figsize=(6, 4.5))
    ...
    # plt.title("changes in calcium concentration(strong HFS)")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-16_3.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    #
    ...
    # plt.title("changes in calcium concentration")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-17.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # # fig = plt.figure(figsize=(10, 5))
    ...
    # # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-18.jpg')
    # # # plt.show()
    #
    # fig = plt.figure(figsize=(10, 5))
    # # membrane potention   ::  neuron with high CREB level
    ...
    # bax.set_xlabel("time (min)")
    # bax.set_title("postneuron firing rate")
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-19.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(5, 5))
    # ax1 = fig.add_subplot(212)
    ...
    # plt.xlim(0, 1000)
    # plt.legend(loc = 1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-20.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(5, 5))
    # ax1 = fig.add_subplot(212)
    # plt.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     V_s[int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + repeat_interval_strongHFS) / step):int(( thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + repeat_interval_strongHFS) / step) + int(1500 / step)], label=' soma', color="blue")
    # plt.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     V_d_1[int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + repeat_interval_strongHFS) / step):int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + repeat_interval_strongHFS) / step) + int(1500 / step)], "--", label=' dend1', color="red")
    # plt.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     V_d_2[int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + repeat_interval_strongHFS) / step):int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + repeat_interval_strongHFS) / step) + int(1500 / step)], "--", label=' dend2', color="black")
    # ax1.set_ylabel("Voltage (mV)")
    # ax1.set_xlabel("time (ms)")
    # plt.title("postneuron spike train")
    # plt.tight_layout(0.1)
    # plt.xlim(0, 1000)
    # plt.legend(loc = 1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-20_1.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(5, 5))
    # ax1 = fig.add_subplot(212)
    # plt.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     V_s[int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + repeat_interval_strongHFS*2) / step):int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + repeat_interval_strongHFS*2) / step) + int(1500 / step)], label=' soma', color="blue")
    # plt.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     V_d_1[int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + repeat_interval_strongHFS*2) / step):int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + repeat_interval_strongHFS*2) / step) + int(1500 / step)], "--", label=' dend1', color="red")
    # plt.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     V_d_2[int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + repeat_interval_strongHFS*2) / step):int((thirty_min + (stimu_Num_weakLFS) * (repeat_interval_weakLFS) + repeat_interval_strongHFS*2) / step) + int(1500 / step)], "--", label=' dend2', color="black")
    # ax1.set_ylabel("Voltage (mV)")
    # ax1.set_xlabel("time (ms)")
    # plt.title("postneuron spike train")
    # plt.tight_layout(0.1)
    # plt.xlim(0, 1000)
    # plt.legend(loc = 1)
    # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1-20_2.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # # plt.show()
    #
    ...
    # # bax.legend(loc=1)
    # # plt.title("changes in calcium concentration")
    # # plt.savefig('cross-capture4_weakLFS+strongHFS_4/1_7_1_1.jpg',bbox_inches = 'tight')
    # # plt.close()
    # plt.show()