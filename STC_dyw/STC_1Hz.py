...
class STC_model():
    def __init__(self):
        ...
        self.alpha_tag = 0.0007  ## s-1  0.0005
        self.beta_tag_synapse = 0
        self.beta_tag_LTP = 1.0  ## s-1   1.0
        self.beta_tag_LTD = 0.2  ## s-1   0.3

        ##calcium threshold for spine
        self.Ca0_synapse = 0.01
        self.Ca1_synapse = 0.2   ## 0.15
        self.Ca0_dendritic = 0.025  ## 0.03
    ...
    def Tag_synapse(self, synapse_Ca_NMDA, tag_synapse_pre):
...
if __name__ == '__main__':
    ...
    ## 30Hz刺激  200ms/1s * 900
    time_spike_train_one = 1000  ## ms  一次刺激时长  200
    preneuron_firing_rate_L = 1  ##Hz
    repeat_times = 1
    repeat_interval = 1000  # ms  重复给予刺激的间隔时间  1000
    stimu_times = 900   ##刺激重复呈现的次数 900
    trail_No = 10 ##第几次实验 1-10(for calulate bar)
    preneuron_spikes_train, preneuron_spikes_time = Poisson_spike_1Hz.poisson_spike_train_L(trail_No-1,stimu_times,time_spike_train_one, preneuron_firing_rate_L, repeat_times,repeat_interval)
    ...
    def calculation():
        ...
        for t_i in range(STC_1.calcium_time_window + 1, len(t_neuron)):
            if t_i * step < stimu_times * (repeat_interval) + STC_1.calcium_time_window*2 :
                ...
                if t_i * step < STC_1.calcium_time_window + stimu_times * repeat_interval + repeat_interval:
            ...
            if V_s[t_i - 1] >= -20 and V_s[t_i - 2] <= -20 and V_s[t_i] >= -20:  ##给刺激的1000ms
            ...
            if np.mod(t_i * step, STC_1.tag_prp_check_window) == 0.0:  # tag——prp时间窗长度,更新Tag/PRP/y/z
                ...
                ## synapse 1 : neuron 1
                tag_synapse[j], tag_flag_synapse[j] = STC_1.Tag_synapse(synapse_Ca_NMDA_average[int(t_i * step)], tag_synapse[j - 1])
                prp[j],prp_x1[j-1],prp_x1[j],prp_x2[j-1],prp_x2[j] = STC_1.PRP(dend2_Ca_channel_average[int(t_i * step)],prp_x1[j - 1], prp_x2[j - 1])
                z_synapse[j], y_synapse[j] = STC_1.z_synapse(prp[j], tag_synapse[j])
                ## 计算发放率
            if t_i * step>1000 and np.mod(t_i * step - STC_1.calcium_time_window, 1000) == 0.0:
                ...
                f = open("1Hz_10/firing rate.txt", "a")
                f.write("firing rate -- spike time (weak HFS) : " + str(spike_time))
                f.write(str(t_i * step) + " ms")
                f.write("\n")
    ...
    resultH5 = h5py.File('1Hz_10.hdf5', 'w')
    ...
    # lns1 = ax1.plot(tag_prp_time / time_scale_tag_prp, z_synapse, label='z_synapse',color='black')
    # lns2 = ax2.plot(tag_prp_time / time_scale_tag_prp, tag_synapse, label='tag_synapse', color='red')
    ...
    # plt.title("synaptic weight changes : None")
    # plt.tight_layout(0.1)
    # plt.savefig('1Hz_10/1.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    # fig = plt.figure(figsize = (10.5,7))
    ...
    # plt.title("synaptic weight changes : None")
    # plt.tight_layout(0.1)
    # plt.savefig('1Hz_10/1_1.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    ...
    # plt.tight_layout(0.1)
    # plt.savefig('1Hz_10/3_tag.jpg', bbox_inches='tight')
    # plt.close()
    #
    #
    #
    ...
    # plt.tight_layout(0.1)
    # plt.savefig('1Hz_10/5_firingrate.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    #
    # soma_Ca_channel_average = np.zeros(5 * 60 * 60 * 1000)
    # dend2_Ca_channel_average = np.zeros(5 * 60 * 60 * 1000) ##5h  1ms为单位
    # synapse_Ca_NMDA_average = np.zeros(5 * 60 * 60 * 1000)
    #
    # for i in range(len(soma_Ca_channel_average[0: int(stimu_times/4) * time_spike_train_one])):
    #     if  i>=100:
    ...
    # lns3 = ax2.plot(t_average[0:1500] - 500, dend2_Ca_channel_average[0:1500], '--',
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('1Hz_10/9_1.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(6, 5))
    # ax1 = fig.add_subplot(212)
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_average[0:1500] - 500, synapse_Ca_NMDA_average[0 + repeat_interval:1500 + repeat_interval],
    #                 label=' synapse Ca NMDA', color="r")
    # lns3 = ax2.plot(t_average[0:1500] - 500, dend2_Ca_channel_average[0 + repeat_interval:1500 + repeat_interval], '--',
    #                 label=' dend Ca channel(Threshold _PRP = 0.01)', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.grid()
    ...
    #     if i > 0 and V_s[i] >= V_s[i - 1] and V_s[i] >= V_s[i + 1] and V_s[i] >= -20:
    #         plt.axvline(x=(i * step - STC_1.calcium_time_window), ls='--', c='y', linewidth=1.0)
    # ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,
    #          synapse_Ca_NMDA[0 + int(repeat_interval / step):int(1500 / step) + int(repeat_interval / step)],
    #          label=' synapse Ca NMDA', color="r")
    # ax2.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,
    #          dend2_Ca_channel[0 + int(repeat_interval / step):int(1500 / step) + int(repeat_interval / step)], '--',
    #          label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca NMDA (μmol/L)", color="r")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('1Hz_10/9_2.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(6, 5))
    # ax1 = fig.add_subplot(212)
    ...
    #                 synapse_Ca_NMDA_average[0 + repeat_interval * 10:1500 + repeat_interval * 10],
    #                 label=' synapse Ca NMDA', color="r")
    # lns3 = ax2.plot(t_average[0:1500] - 500,
    #                 dend2_Ca_channel_average[0 + repeat_interval * 10:1500 + repeat_interval * 10], '--',
    #                 label=' dend Ca channel(Threshold _PRP = 0.01)', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.grid()
    ...
    #         plt.axvline(x=(i * step - STC_1.calcium_time_window), ls='--', c='y', linewidth=1.0)
    # ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,
    #          synapse_Ca_NMDA[0 + int(repeat_interval * 10 / step):int(1500 / step) + int(repeat_interval * 10 / step)],
    #          label=' synapse Ca NMDA', color="r")
    # ax2.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,
    #          dend2_Ca_channel[0 + int(repeat_interval * 10 / step):int(1500 / step) + int(repeat_interval * 10 / step)],
    #          '--', label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca NMDA (μmol/L)", color="r")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('1Hz_10/9_10.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(6, 5))
    # ax1 = fig.add_subplot(212)
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_average[0:1500] - 500,
    #                 synapse_Ca_NMDA_average[0 + repeat_interval * 100:1500 + repeat_interval * 100],
    #                 label=' synapse Ca NMDA', color="r")
    # lns3 = ax2.plot(t_average[0:1500] - 500,
    #                 dend2_Ca_channel_average[0 + repeat_interval * 100:1500 + repeat_interval * 100], '--',
    #                 label=' dend Ca channel(Threshold _PRP = 0.01)', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.grid()
    ...
    #     if i > 0 and V_s[i] >= V_s[i - 1] and V_s[i] >= V_s[i + 1] and V_s[i] >= -20:
    #         plt.axvline(x=(i * step - STC_1.calcium_time_window), ls='--', c='y', linewidth=1.0)
    # ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window, synapse_Ca_NMDA[
    #                                                                    0 + int(repeat_interval * 100 / step):int(
    #                                                                        1500 / step) + int(
    #                                                                        repeat_interval * 100 / step)],
    #          label=' synapse Ca NMDA', color="r")
    # ax2.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window, dend2_Ca_channel[
    #                                                                    0 + int(repeat_interval * 100 / step):int(
    #                                                                        1500 / step) + int(
    #                                                                        repeat_interval * 100 / step)], '--',
    #          label=' dend Ca channel', color="black")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('1Hz_10/9_100.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(10.5, 7))
    # ax1 = fig.add_subplot(212)
    ...
    # ax1.legend(loc=1)
    # plt.tight_layout(0.1)
    # plt.savefig('1Hz_10/5.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    #
    ...
    # plt.title("changes in calcium concentration")
    # plt.tight_layout(0.1)
    # plt.savefig('1Hz_10/6.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize = (10.5,7))
    # ax1 = fig.add_subplot(211)
    ...
    # plt.title("changes in calcium concentration")
    # plt.tight_layout(0.1)
    # plt.savefig('1Hz_10/6_1.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize = (10.5,7))
    # ax1 = fig.add_subplot(211)
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_neuron[0:int(1500/step)] - 500, synapse_Ca_NMDA[0 + int(repeat_interval*100 / step):int(1500/step) + int(repeat_interval*100 / step)], label=' synapse_Ca_NMDA',color="r")
    # lns3 = ax2.plot(t_neuron[0:int(1500/step)] - 500, dend2_Ca_channel[0 + int(repeat_interval*100 / step):int(1500/step) + int(repeat_interval*100 / step)], '--', label=' dend_Ca_channel',color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.grid()
    # ax1.set_ylabel("synapse_Ca_NMDA (μmol/L)", color="r")
    ...
    # plt.title("changes in calcium concentration")
    # plt.tight_layout(0.1)
    # plt.savefig('1Hz_10/6_100.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize = (10.5,7))
    # ax1 = fig.add_subplot(211)
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_neuron[0:int(1500/step)] - 500, synapse_Ca_NMDA[0 + int(repeat_interval*200 / step):int(1500/step) + int(repeat_interval*200 / step)], label=' synapse_Ca_NMDA',color="r")
    # lns3 = ax2.plot(t_neuron[0:int(1500/step)] - 500, dend2_Ca_channel[0 + int(repeat_interval*200 / step):int(1500/step) + int(repeat_interval*200 / step)], '--', label=' dend_Ca_channel',color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.grid()
    # ax1.set_ylabel("synapse_Ca_NMDA (μmol/L)", color="r")
    ...
    # plt.title("changes in calcium concentration")
    # plt.tight_layout(0.1)
    # plt.savefig('1Hz_10/6_200.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    #
    # # fig = plt.figure(figsize=(10, 5))
    ...
    # # bax.legend(loc=1)
    # # plt.title("changes in calcium concentration")
    # # plt.savefig('1Hz_10/7_1_1.jpg',bbox_inches = 'tight')
    # # plt.close()
    #
    #
    # # fig = plt.figure(figsize=(10.5, 7))
