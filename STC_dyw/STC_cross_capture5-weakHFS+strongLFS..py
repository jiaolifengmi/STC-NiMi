...
if __name__ == '__main__':
    ...
    print("strongLFS number of pulses : " + str(np.shape(preneuron_spikes_time_strongLFS)))  ## (900, 20)
    ...
    def calculation():
        ...
        for t_i in range(STC_1.calcium_time_window+1,len(t_neuron)):
            if t_i * step < (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + STC_1.calcium_time_window*2:
                ...
                synapse_Ca_NMDA_weakHFS[t_i],I_nmda_weakHFS_now = synapse_weakHFS.NMDA_Ca(t_i,synapse_Ca_NMDA_weakHFS[t_i - 1],V_d_2[t_i - 1],g_list_nmda_weakHFS,(np.array(preneuron_spikes_time_weakHFS[int(count)]) + repeat_interval * count +STC_1.calcium_time_window).tolist())
                I_ampa_weakHFS_now = synapse_weakHFS.I_AMPA(t_i,V_d_2[t_i],g_list_ampa_weakHFS,(np.array(preneuron_spikes_time_weakHFS[int(count)]) + repeat_interval * count +STC_1.calcium_time_window).tolist(),z_synapse_1[int(np.ceil((t_i*step) / STC_1.tag_prp_check_window))-1])
            ...
            elif t_i * step < STC_1.calcium_time_window*2  + repeat_interval_weakHFS * stimu_Num_weakHFS + thirty_min + stimu_Num_strongLFS * repeat_interval_strongLFS\
                    and t_i * step >= thirty_min + STC_1.calcium_time_window + repeat_interval_weakHFS * stimu_Num_weakHFS:  ##30min 后给另一个刺激
                time_ = 2
                count = int((t_i * step - STC_1.calcium_time_window - thirty_min - stimu_Num_weakHFS * repeat_interval_weakHFS) / repeat_interval_strongLFS)  ##0-n次刺激,repeat_interval_strongLFS = 10min/1次
                ...
                ## strongLFS
                V_s[t_i],V_d_1[t_i],V_d_2[t_i],dend2_Ca_channel[t_i] = voltage.V_update(t_i,V_s[t_i-1],V_d_1[t_i-1],V_d_2[t_i-1])
                synapse_Ca_NMDA_strongLFS[t_i],I_nmda_strongLFS_now = synapse_strongLFS.NMDA_Ca(t_i,synapse_Ca_NMDA_strongLFS[t_i - 1],V_d_2[t_i - 1],g_list_nmda_strongLFS,(np.array(preneuron_spikes_time_strongLFS[count])+ thirty_min + repeat_interval_strongLFS * count +STC_1.calcium_time_window + (stimu_Num_weakHFS) * (repeat_interval_weakHFS)).tolist())
                I_ampa_strongLFS_now = synapse_strongLFS.I_AMPA(t_i,V_d_2[t_i],g_list_ampa_strongLFS,(np.array(preneuron_spikes_time_strongLFS[int(count)])+ thirty_min + repeat_interval_strongLFS * count + STC_1.calcium_time_window + (stimu_Num_weakHFS) * (repeat_interval_weakHFS)).tolist(),z_synapse_2[int(np.ceil((t_i*step) / STC_1.tag_prp_check_window))-1])
                voltage.I_syn_d2 = I_nmda_weakHFS_now + I_ampa_weakHFS_now + I_nmda_strongLFS_now + I_ampa_strongLFS_now
            ...
            if t_i * step>1000 and np.mod(t_i * step - STC_1.calcium_time_window, 1000) == 0.0:
                ...
                f = open("cross-capture5_weakHFS+strongLFS_5/firing rate.txt", "a")
        ...
        f = open("cross-capture5_weakHFS+strongLFS_5/运行时间.txt", "a")
    ...
    resultH5 = h5py.File('cross-capture5_weakHFS+strongLFS_5.hdf5', 'w')
    resultH5.create_dataset(name='V_s', data=(np.array(V_s[0:int((STC_1.calcium_time_window*2 + stimu_Num_weakHFS * repeat_interval + thirty_min + int((stimu_Num_strongLFS) * repeat_interval_strongLFS))/step)])).tolist())
    resultH5.create_dataset(name='V_d_1', data=(np.array(V_d_1[0:int((STC_1.calcium_time_window*2 + stimu_Num_weakHFS * repeat_interval + thirty_min + int((stimu_Num_strongLFS) * repeat_interval_strongLFS))/step)])).tolist())
    resultH5.create_dataset(name='V_d_2', data=(np.array(V_d_2[0:int((STC_1.calcium_time_window*2 + stimu_Num_weakHFS * repeat_interval + thirty_min + int((stimu_Num_strongLFS) * repeat_interval_strongLFS))/step)])).tolist())
    resultH5.create_dataset(name='soma_Ca_channel', data=(np.array(soma_Ca_channel[0:int((STC_1.calcium_time_window*2 + stimu_Num_weakHFS * repeat_interval + thirty_min + int((stimu_Num_strongLFS) * repeat_interval_strongLFS))/step)])).tolist())
    resultH5.create_dataset(name='dend_Ca_channel', data=(np.array(dend2_Ca_channel[0:int((STC_1.calcium_time_window*2 + stimu_Num_weakHFS * repeat_interval + thirty_min + int((stimu_Num_strongLFS) * repeat_interval_strongLFS))/step)])).tolist())
    resultH5.create_dataset(name='synapse_Ca_NMDA_strongLFS', data=(np.array(synapse_Ca_NMDA_strongLFS[0:int((STC_1.calcium_time_window*2 + stimu_Num_weakHFS * repeat_interval + thirty_min + int((stimu_Num_strongLFS) * repeat_interval_strongLFS))/step)])).tolist())
    resultH5.create_dataset(name='synapse_Ca_NMDA_weakHFS', data=(np.array(synapse_Ca_NMDA_weakHFS[0:int((STC_1.calcium_time_window*2 + stimu_Num_weakHFS * repeat_interval + thirty_min + int((stimu_Num_strongLFS) * repeat_interval_strongLFS))/step)])).tolist())
    ...
    resultH5.create_dataset(name='z_synapse_strongLFS', data=(np.array(z_synapse_1)).tolist())
    resultH5.create_dataset(name='z_synapse_weakHFS', data=(np.array(z_synapse_2)).tolist())
    resultH5.create_dataset(name='prp', data=(np.array(prp)).tolist())
    resultH5.create_dataset(name='tag_synapse_strongLFS', data=(np.array(tag_synapse_1)).tolist())
    resultH5.create_dataset(name='tag_synapse_weakHFS', data=(np.array(tag_synapse_2)).tolist())
    resultH5.create_dataset(name='y_synapse_strongLFS', data=(np.array(y_synapse_1)).tolist())
    resultH5.create_dataset(name='y_synapse_weakHFS', data=(np.array(y_synapse_2)).tolist())
    resultH5.create_dataset(name='tag_flag_strongLFS', data=(np.array(tag_flag_synapse_1)).tolist())
    resultH5.create_dataset(name='tag_flag_weakHFS', data=(np.array(tag_flag_synapse_2)).tolist())
    ...
    # ax4 = ax3.twinx()
    # lns66 = ax3.plot(tag_prp_time / time_scale_tag_prp, z_synapse_2, label='z synapse(strong LFS stimuli)',color='black')  ##familiar
    ...
    # plt.savefig('cross-capture5_weakHFS+strongLFS_5/1-1_prp1000.jpg',bbox_inches = 'tight')
    # plt.close()
    # # # plt.show()
    #
    # # Visualization
    ...
    # plt.title(" synaptic weight changes")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture5_weakHFS+strongLFS_5/1-1_1_prp1000.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(6, 5))
    # ax3 = fig.add_subplot(211)
    ...
    # plt.title(" synaptic weight changes")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture5_weakHFS+strongLFS_5/1-1_2_prp100.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(6, 5))
    # ax3 = fig.add_subplot(211)
    ...
    # plt.title(" synaptic weight changes")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture5_weakHFS+strongLFS_5/1-1_3_prp10.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    # fig = plt.figure(figsize=(6, 5))
    ...
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture5_weakHFS+strongLFS_5/1-3_prp100.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    #
    #
    ...
    # # plt.tight_layout(0.1)
    # # plt.savefig('cross-capture5/1-7.jpg')
    # # # # plt.show()
    #
    # step = 0.05
    ...
    #
    # for i in range(STC_1.calcium_time_window + stimu_Num_weakHFS * repeat_interval_weakHFS + thirty_min + stimu_Num_strongLFS * repeat_interval_strongLFS + repeat_interval):
    #     if i < STC_1.calcium_time_window + stimu_Num_weakHFS *repeat_interval_weakHFS + repeat_interval\
    #         or (i >=  STC_1.calcium_time_window + stimu_Num_weakHFS *repeat_interval_weakHFS + thirty_min):
    #         if i >= 100:
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture5_weakHFS+strongLFS_5/1-Ca_average_weakHFS_1.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    #
    ...
    #                 synapse_Ca_NMDA_strongLFS_average[thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS):int(thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + 1500)],
    #                 label=' synapse Ca NMDA-strong LFS stimuli ', color="r")
    ...
    #     synapse_Ca_NMDA_strongLFS[int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS)) / step):int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS)) / step) + int(1500 / step)],
    #     label=' synapse Ca NMDA-strong LFS stimuli', color="r")
    # lns2 = ax1.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture5_weakHFS+strongLFS_5/1-Ca_average_strongLFS_1.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # # plt.show()
    # fig = plt.figure(figsize=(5.8, 4.0))
    ...
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_average[thirty_min:int(thirty_min + 1500)] - thirty_min - 500,
    #                 synapse_Ca_NMDA_strongLFS_average[thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS)+repeat_interval_strongLFS:int(thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) +repeat_interval_strongLFS + 1500)],
    #                 label=' synapse Ca NMDA-strong LFS stimuli', color="r")
    # lns2 = ax1.plot(t_average[thirty_min:int(thirty_min + 1500)] - thirty_min - 500,
    #                 synapse_Ca_NMDA_weakHFS_average[thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS:int(thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS + 1500)],
    #                 label='  synapse Ca NMDA-weak HFS stimuli', color="b")
    # lns3 = ax2.plot(t_average[thirty_min:int(thirty_min + 1500)] - thirty_min - 500,
    #                 dend2_Ca_channel_average[thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS:int(thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS + 1500)], '--',
    #                 label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca NMDA (μmol/L)", color="black")
    ...
    # ax1 = fig.add_subplot(211)
    # ax2 = ax1.twinx()
    # for i in range(len(V_s[int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS) / step):int(((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS) + 1500) / step)]) - 1):
    #     if i > 0 and V_s[i + int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS) / step)] >= V_s[i + int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS) / step) - 1] and V_s[
    #         i + int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS) / step)] >= V_s[i + int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS) / step) + 1] and V_s[
    #         i + int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS) / step)] >= -40:
    #         plt.axvline(x=(i * step - STC_1.calcium_time_window), ls='--', c='y', linewidth=1.0)
    # lns1 = ax1.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     synapse_Ca_NMDA_strongLFS[int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS) / step):int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS) / step) + int(1500 / step)],
    #     label=' synapse Ca NMDA-strong LFS stimuli', color="r")
    # lns2 = ax1.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     synapse_Ca_NMDA_weakHFS[int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS) / step):int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS) / step) + int(1500 / step)],
    #     label='  synapse Ca NMDA-weak HFS stimuli', color="b")
    # lns3 = ax2.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     dend2_Ca_channel[int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS) / step):int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS) / step) + int(1500 / step)], '--',
    #     label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca_NMDA (μmol/L)", color="black")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture5_weakHFS+strongLFS_5/1-Ca_average_strongLFS_2.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    # fig = plt.figure(figsize=(5.8, 4.0))
    ...
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_average[thirty_min:int(thirty_min + 1500)] - thirty_min - 500,
    #                 synapse_Ca_NMDA_strongLFS_average[thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS)+repeat_interval_strongLFS*400:int(thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) +repeat_interval_strongLFS*400 + 1500)],
    #                 label=' synapse Ca NMDA-strong LFS stimuli', color="r")
    # lns2 = ax1.plot(t_average[thirty_min:int(thirty_min + 1500)] - thirty_min - 500,
    #                 synapse_Ca_NMDA_weakHFS_average[thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS*400:int(thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS*400 + 1500)],
    #                 label='  synapse Ca NMDA-weak HFS stimuli', color="b")
    # lns3 = ax2.plot(t_average[thirty_min:int(thirty_min + 1500)] - thirty_min - 500,
    #                 dend2_Ca_channel_average[thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS*400:int(thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS*400 + 1500)], '--',
    #                 label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca NMDA (μmol/L)", color="black")
    ...
    # ax1 = fig.add_subplot(211)
    # ax2 = ax1.twinx()
    # for i in range(len(V_s[int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS*400) / step):int(((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS*400) + 1500) / step)]) - 1):
    #     if i > 0 and V_s[i + int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS*400) / step)] >= V_s[i + int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS*400) / step) - 1] and V_s[
    #         i + int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS*400) / step)] >= V_s[i + int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS*400) / step) + 1] and V_s[
    #         i + int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS*400) / step)] >= -40:
    #         plt.axvline(x=(i * step - STC_1.calcium_time_window), ls='--', c='y', linewidth=1.0)
    # lns1 = ax1.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     synapse_Ca_NMDA_strongLFS[int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS*400) / step):int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS*400) / step) + int(1500 / step)],
    #     label=' synapse Ca NMDA-strong LFS stimuli', color="r")
    # lns2 = ax1.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     synapse_Ca_NMDA_weakHFS[int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS*400) / step):int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS*400) / step) + int(1500 / step)],
    #     label='  synapse Ca NMDA-weak HFS stimuli', color="b")
    # lns3 = ax2.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     dend2_Ca_channel[int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS*400) / step):int((thirty_min + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) + repeat_interval_strongLFS*400) / step) + int(1500 / step)], '--',
    #     label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca_NMDA (μmol/L)", color="black")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture5_weakHFS+strongLFS_5/1-Ca_average_strongLFS_3.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    ...
    # plt.savefig('cross-capture5_weakHFS+strongLFS_5/1-8.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    ...
    # plt.legend(loc = 1)
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture5_weakHFS+strongLFS_5/1-9.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    #
    #
    ...
    # # plt.savefig('cross-capture5_weakHFS+strongLFS_5/1-18.jpg')
    # # # plt.show()
    #
    # fig = plt.figure(figsize=(10, 5))
    # # membrane potention   ::  neuron with high CREB level
    ...
    # bax.set_xlabel("time (min)")
    # bax.set_title("postneuron firing rate")
    # plt.savefig('cross-capture5_weakHFS+strongLFS_5/1-19.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(5, 5))
    # ax1 = fig.add_subplot(212)
    ...
    # plt.xlim(0, 240)
    # plt.legend(loc = 1)
    # plt.savefig('cross-capture5_weakHFS+strongLFS_5/1-20.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(5, 5))
    # ax1 = fig.add_subplot(212)
    # plt.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     V_s[int((thirty_min + repeat_interval_strongLFS + (stimu_Num_weakHFS) * (repeat_interval_weakHFS)) / step):int(( thirty_min + repeat_interval_strongLFS+ (stimu_Num_weakHFS) * (repeat_interval_weakHFS)) / step) + int(1500 / step)], label=' soma', color="blue")
    # plt.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     V_d_1[int((thirty_min + repeat_interval_strongLFS + (stimu_Num_weakHFS) * (repeat_interval_weakHFS)) / step):int((thirty_min + repeat_interval_strongLFS + (stimu_Num_weakHFS) * (repeat_interval_weakHFS)) / step) + int(1500 / step)], "--", label=' dend1', color="red")
    # plt.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     V_d_2[int((thirty_min + repeat_interval_strongLFS + (stimu_Num_weakHFS) * (repeat_interval_weakHFS)) / step):int((thirty_min + repeat_interval_strongLFS + (stimu_Num_weakHFS) * (repeat_interval_weakHFS)) / step) + int(1500 / step)], "--", label=' dend2', color="black")
    # ax1.set_ylabel("Voltage (mV)")
    # ax1.set_xlabel("time (ms)")
    # plt.title("postneuron spike train")
    # plt.tight_layout(0.1)
    # plt.xlim(0, 240)
    # plt.legend(loc = 1)
    # plt.savefig('cross-capture5_weakHFS+strongLFS_5/1-20_2.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(5, 5))
    # ax1 = fig.add_subplot(212)
    # plt.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     V_s[int((thirty_min + repeat_interval_strongLFS*100 + (stimu_Num_weakHFS) * (repeat_interval_weakHFS)) / step):int(( thirty_min + repeat_interval_strongLFS*100+ (stimu_Num_weakHFS) * (repeat_interval_weakHFS)) / step) + int(1500 / step)], label=' soma', color="blue")
    # plt.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     V_d_1[int((thirty_min + repeat_interval_strongLFS*100 + (stimu_Num_weakHFS) * (repeat_interval_weakHFS)) / step):int((thirty_min + repeat_interval_strongLFS*100 + (stimu_Num_weakHFS) * (repeat_interval_weakHFS)) / step) + int(1500 / step)], "--", label=' dend1', color="red")
    # plt.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     V_d_2[int((thirty_min + repeat_interval_strongLFS*100 + (stimu_Num_weakHFS) * (repeat_interval_weakHFS)) / step):int((thirty_min + repeat_interval_strongLFS*100 + (stimu_Num_weakHFS) * (repeat_interval_weakHFS)) / step) + int(1500 / step)], "--", label=' dend2', color="black")
    # ax1.set_ylabel("Voltage (mV)")
    # ax1.set_xlabel("time (ms)")
    # plt.title("postneuron spike train")
    # plt.tight_layout(0.1)
    # plt.xlim(0, 240)
    # plt.legend(loc = 1)
    # plt.savefig('cross-capture5_weakHFS+strongLFS_5/1-20_3.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    ...
    # # bax.legend(loc=1)
    # # plt.title("changes in calcium concentration")
    # # plt.savefig('cross-capture5_weakHFS+strongLFS_5/1_7_1_1.jpg',bbox_inches = 'tight')
    # # plt.close()
    # plt.show()