...
if __name__ == '__main__':
    ...
    synapse_weakHFS = synapases();synapse_strongLFS = synapases()

    STC_1 = STC_model()
    STC_2 = STC_model()
    synapse_strongLFS.G_AMPA = 1.6  ## strongLFS 20Hz
    ...
    time_spike_train_one_weakHFS = 200  ## ms  一次刺激时长  1000
    time_spike_train_one_strongLFS = 150  ## ms  一次刺激时长  1000
    ...
    print("weakHFS number of pulses : " + str(np.shape(preneuron_spikes_time_weakHFS)))  ## (900, 1)
    print("strongLFS number of pulses : " + str(np.shape(preneuron_spikes_time_strongLFS)))  ## (3, 20)
    ...
    synapse_Ca_NMDA_weakHFS = np.zeros(len(t_neuron));   synapse_Ca_NMDA_strongLFS = np.zeros(len(t_neuron))
    ...
    def calculation():
        ...
        for t_i in range(STC_1.calcium_time_window+1,len(t_neuron)):
            if t_i * step < STC_1.calcium_time_window + stimu_Num_strongLFS * repeat_interval_strongLFS + repeat_interval:
                time_ = 1
                count = int((t_i * step - STC_1.calcium_time_window) / repeat_interval_strongLFS)  ##0-n次刺激,repeat_interval_strongLFS = 10min/1次
                ...
                synapse_Ca_NMDA_strongLFS[t_i],I_nmda_strongLFS_now = synapse_strongLFS.NMDA_Ca(t_i,synapse_Ca_NMDA_strongLFS[t_i - 1],V_d_2[t_i - 1],g_list_nmda_strongLFS,(np.array(preneuron_spikes_time_strongLFS[count]) + repeat_interval_strongLFS * count +STC_1.calcium_time_window).tolist())
                I_ampa_strongLFS_now = synapse_strongLFS.I_AMPA(t_i,V_d_2[t_i],g_list_ampa_strongLFS,(np.array(preneuron_spikes_time_strongLFS[int(count)]) + repeat_interval_strongLFS * count +STC_1.calcium_time_window).tolist(),z_synapse_2[int(np.ceil((t_i*step) / STC_1.tag_prp_check_window))-1])
                voltage.I_syn_d2 = I_nmda_weakHFS_now + I_ampa_weakHFS_now + I_nmda_strongLFS_now + I_ampa_strongLFS_now
                I_ampa_strongLFS_pre = I_ampa_strongLFS_now
                I_nmda_strongLFS_pre = I_nmda_strongLFS_now
            ##30min 后给另一个刺激
            elif t_i * step < thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + STC_1.calcium_time_window*2 + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) \
                    and t_i * step >= thirty_min + STC_1.calcium_time_window + (stimu_Num_strongLFS) * (repeat_interval_strongLFS):
                time_ = 2
                count = int((t_i * step - STC_1.calcium_time_window - thirty_min - (stimu_Num_strongLFS) * (repeat_interval_strongLFS)) / repeat_interval_weakHFS)  ##0-n次刺激,repeat_interval = 1000ms/1次
                ...
                ### weakHFS  900*1s
                V_s[t_i],V_d_1[t_i],V_d_2[t_i],dend2_Ca_channel[t_i] = voltage.V_update(t_i,V_s[t_i-1],V_d_1[t_i-1],V_d_2[t_i-1])
                synapse_Ca_NMDA_weakHFS[t_i],I_nmda_weakHFS_now = synapse_weakHFS.NMDA_Ca(t_i,synapse_Ca_NMDA_weakHFS[t_i - 1],V_d_2[t_i - 1],g_list_nmda_weakHFS,(np.array(preneuron_spikes_time_weakHFS[int(count)])+ thirty_min  + STC_1.calcium_time_window + repeat_interval * count + (stimu_Num_strongLFS) * (repeat_interval_strongLFS)).tolist())
                I_ampa_weakHFS_now = synapse_weakHFS.I_AMPA(t_i,V_d_2[t_i],g_list_ampa_weakHFS,(np.array(preneuron_spikes_time_weakHFS[int(count)])+thirty_min + STC_1.calcium_time_window + repeat_interval * count + (stimu_Num_strongLFS) * (repeat_interval_strongLFS)).tolist(),z_synapse_1[int(np.ceil((t_i*step) / STC_1.tag_prp_check_window))-1])
                voltage.I_syn_d2 = I_nmda_weakHFS_now + I_ampa_weakHFS_now + I_nmda_strongLFS_now + I_ampa_strongLFS_now
            ...
            if np.mod(t_i*step, STC_1.tag_prp_check_window) == 0.0:  # tag——prp时间窗长度,更新Tag/PRP/y/z
                ...
                print('syn_strongLFS、syn_weakHFS、dend')
                ...
                ## synapse 1 : weakHFS      synapse 2 : strongLFS
                tag_synapse_2[j], tag_flag_synapse_2[j] = STC_2.Tag_synapse(synapse_Ca_NMDA_strongLFS_average[int(t_i*step)], tag_synapse_2[j - 1],STC_1.dopamine_nonovel)
                tag_synapse_1[j], tag_flag_synapse_1[j] = STC_1.Tag_synapse(synapse_Ca_NMDA_weakHFS_average[int(t_i*step)],tag_synapse_1[j - 1], STC_2.dopamine_nonovel)
            ...
            if t_i * step>1000 and np.mod(t_i * step - STC_1.calcium_time_window, 1000) == 0.0:
                ...
                f = open("cross-capture6_strongLFS+weakHFS_3/firing rate.txt", "a")
        ...
        f = open("cross-capture6_strongLFS+weakHFS_3/运行时间.txt", "a")
    ...
    resultH5 = h5py.File('cross-capture6_strongLFS+weakHFS_3.hdf5', 'w')
    resultH5.create_dataset(name='V_s', data=(np.array(V_s[0:int((STC_1.calcium_time_window*2 + (stimu_Num_strongLFS) * repeat_interval_strongLFS + thirty_min + stimu_Num_weakHFS * repeat_interval_weakHFS)/step)])).tolist())
    resultH5.create_dataset(name='V_d_1', data=(np.array(V_d_1[0:int((STC_1.calcium_time_window*2 + (stimu_Num_strongLFS) * repeat_interval_strongLFS + thirty_min + stimu_Num_weakHFS * repeat_interval_weakHFS)/step)])).tolist())
    resultH5.create_dataset(name='V_d_2', data=(np.array(V_d_2[0:int((STC_1.calcium_time_window*2 + (stimu_Num_strongLFS) * repeat_interval_strongLFS + thirty_min + stimu_Num_weakHFS * repeat_interval_weakHFS)/step)])).tolist())
    resultH5.create_dataset(name='soma_Ca_channel', data=(np.array(soma_Ca_channel[0:int((STC_1.calcium_time_window*2 + (stimu_Num_strongLFS) * repeat_interval_strongLFS + thirty_min + stimu_Num_weakHFS * repeat_interval_weakHFS)/step)])).tolist())
    resultH5.create_dataset(name='dend_Ca_channel', data=(np.array(dend2_Ca_channel[0:int((STC_1.calcium_time_window*2 + (stimu_Num_strongLFS) * repeat_interval_strongLFS + thirty_min + stimu_Num_weakHFS * repeat_interval_weakHFS)/step)])).tolist())
    resultH5.create_dataset(name='synapse_Ca_NMDA_strongLFS', data=(np.array(synapse_Ca_NMDA_strongLFS[0:int((STC_1.calcium_time_window*2 + (stimu_Num_strongLFS) * repeat_interval_strongLFS + thirty_min + stimu_Num_weakHFS * repeat_interval_weakHFS)/step)])).tolist())
    resultH5.create_dataset(name='synapse_Ca_NMDA_weakHFS', data=(np.array(synapse_Ca_NMDA_weakHFS[0:int((STC_1.calcium_time_window*2 + (stimu_Num_strongLFS) * repeat_interval_strongLFS + thirty_min + stimu_Num_weakHFS * repeat_interval_weakHFS)/step)])).tolist())
    ...
    # resultH5.create_dataset(name='synapse_Ca_NMDA_weakHFS_sub', data=(np.array(synapse_Ca_NMDA_weakHFS_sub)).tolist())
    resultH5.create_dataset(name='z_synapse_strongLFS', data=(np.array(z_synapse_1)).tolist())### strong 和 weal的z、tag、prp、y数据存反了，后期再进行训练需要更改一下
    resultH5.create_dataset(name='z_synapse_weakHFS', data=(np.array(z_synapse_2)).tolist())### strong 和 weal的z、tag、prp、y数据存反了，后期再进行训练需要更改一下
    resultH5.create_dataset(name='prp', data=(np.array(prp)).tolist())
    resultH5.create_dataset(name='tag_synapse_strongLFS', data=(np.array(tag_synapse_1)).tolist())### strong 和 weal的z、tag、prp、y数据存反了，后期再进行训练需要更改一下
    resultH5.create_dataset(name='tag_synapse_weakHFS', data=(np.array(tag_synapse_2)).tolist())### strong 和 weal的z、tag、prp、y数据存反了，后期再进行训练需要更改一下
    resultH5.create_dataset(name='y_synapse_strongLFS', data=(np.array(y_synapse_1)).tolist())### strong 和 weal的z、tag、prp、y数据存反了，后期再进行训练需要更改一下
    resultH5.create_dataset(name='y_synapse_weakHFS', data=(np.array(y_synapse_2)).tolist())### strong 和 weal的z、tag、prp、y数据存反了，后期再进行训练需要更改一下
    resultH5.create_dataset(name='tag_flag_strongLFS', data=(np.array(tag_flag_synapse_1)).tolist())### strong 和 weal的z、tag、prp、y数据存反了，后期再进行训练需要更改一下
    resultH5.create_dataset(name='tag_flag_weakHFS', data=(np.array(tag_flag_synapse_2)).tolist())### strong 和 weal的z、tag、prp、y数据存反了，后期再进行训练需要更改一下
    ...
    # ax4 = ax3.twinx()
    # lns66 = ax3.plot(tag_prp_time / time_scale_tag_prp, z_synapse_2,label='z synapse(strong LFS)',color='black')
    # lns6 = ax3.plot(tag_prp_time / time_scale_tag_prp, z_synapse_1,  '--', label='z synapse(weak HFS)',color='black')
    # lns33 = ax4.plot(tag_prp_time / time_scale_tag_prp, tag_synapse_2, label='tag synapse(strong LFS)',color='red')
    # lns3 = ax4.plot(tag_prp_time / time_scale_tag_prp, tag_synapse_1, '--', label='tag synapse(weak HFS)', color='red')
    # lns4 = ax4.plot(tag_prp_time / time_scale_tag_prp, prp*1000, label='PRP', color='b')
    # # lns5 = ax4.plot(tag_prp_time / time_scale_tag_prp, y_synapse_1, label='y_synapse_strongLFS', color='y')
    # # lns55 = ax4.plot(tag_prp_time / time_scale_tag_prp, y_synapse_2, '--', label='y_synapse_weakHFS',color='y')
    ...
    # plt.savefig('cross-capture6_strongLFS+weakHFS_3/1-1_prp1000.jpg',bbox_inches = 'tight')
    # plt.close()
    # # # plt.show()
    #
    # # Visualization
    ...
    # plt.title(" synaptic weight changes")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture6_strongLFS+weakHFS_3/1-1_1_prp1000.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # # Visualization
    # fig = plt.figure(figsize=(6, 5))
    ...
    # lns6 = ax3.plot(tag_prp_time / time_scale_tag_prp, z_synapse_1,  '--', label='z synapse(weak HFS)',color='black')
    # lns33 = ax4.plot(tag_prp_time / time_scale_tag_prp, tag_synapse_2, label='tag synapse(strong LFS)',color='red')
    # lns3 = ax4.plot(tag_prp_time / time_scale_tag_prp, tag_synapse_1*10, '--', label='tag synapse(weak HFS)', color='red')
    # lns4 = ax4.plot(tag_prp_time / time_scale_tag_prp, prp*1000, label='PRP', color='b')
    # # lns5 = ax4.plot(tag_prp_time / time_scale_tag_prp, y_synapse_1, label='y_synapse_strongLFS', color='y')
    # # lns55 = ax4.plot(tag_prp_time / time_scale_tag_prp, y_synapse_2, '--', label='y_synapse_weakHFS',color='y')
    # ax4.set_ylabel("Amount(tag PRP)")
    ...
    # plt.title(" synaptic weight changes")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture6_strongLFS+weakHFS_3/1-1_1_tag_synapse_weakHFS10 prp1000.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(6, 5))
    # ax3 = fig.add_subplot(211)
    ...
    # plt.title(" synaptic weight changes")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture6_strongLFS+weakHFS_3/1-1_2_prp100.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(6, 5))
    # ax3 = fig.add_subplot(211)
    ...
    # plt.title(" synaptic weight changes")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture6_strongLFS+weakHFS_3/1-1_3_prp10.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    # fig = plt.figure(figsize=(6, 5))
    ...
    # plt.title("synaptic weight changes(synapse1 weak HFS)")
    # plt.tight_layout(0.1)
    ...
    # labs = [l.get_label() for l in lns]
    # ax1.legend(lns, labs, loc=1)
    # plt.title("synaptic weight changes(synapse2 strong LFS)")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture6_strongLFS+weakHFS_3/1-3_prp100.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    #
    ...
    # # plt.tight_layout(0.1)
    # # plt.savefig('cross-capture6/1-7.jpg')
    # # # # plt.show()
    #
    # step = 0.05
    ...
    #
    # for i in range(STC_1.calcium_time_window + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + thirty_min + stimu_Num_weakHFS * repeat_interval_weakHFS + repeat_interval):
    #     if i < STC_1.calcium_time_window + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + repeat_interval\
    #         or (i >=  STC_1.calcium_time_window + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + thirty_min):
    #         if i >= 100:
    ...
    # ax1 = fig.add_subplot(212)
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_average[0:1500] - STC_1.calcium_time_window, synapse_Ca_NMDA_strongLFS_average[0:1500],label=' synapse Ca NMDA-strong LFS', color="r")
    # lns2 = ax1.plot(t_average[0:1500] - STC_1.calcium_time_window, synapse_Ca_NMDA_weakHFS_average[0:1500],label='  synapse Ca NMDA-weak HFS', color="b")
    ...
    # lns1 = ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,synapse_Ca_NMDA_strongLFS[0:int(1500 / step)],label=' synapse Ca NMDA-strong LFS', color="r")
    # lns2 = ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,synapse_Ca_NMDA_weakHFS[0:int(1500 / step)],label=' synapse Ca NMDA-weak HFS', color="b")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture6_strongLFS+weakHFS_3/1-Ca_average_strongLFS_1.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    #
    ...
    # ax1 = fig.add_subplot(212)
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_average[0:1500] - STC_1.calcium_time_window, synapse_Ca_NMDA_strongLFS_average[0 + repeat_interval_strongLFS:1500 + repeat_interval_strongLFS],label=' synapse Ca NMDA-strong LFS', color="r")
    # lns2 = ax1.plot(t_average[0:1500] - STC_1.calcium_time_window, synapse_Ca_NMDA_weakHFS_average[0 + repeat_interval_strongLFS:1500 + repeat_interval_strongLFS],label='  synapse Ca NMDA-weak HFS', color="b")
    # lns3 = ax2.plot(t_average[0:1500] - STC_1.calcium_time_window, dend2_Ca_channel_average[0 + repeat_interval_strongLFS:1500 + repeat_interval_strongLFS], '--',label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca NMDA (μmol/L)", color="black")
    # ax2.set_ylabel('dend Ca channel (μmol/L)', color="black")
    ...
    # ax1 = fig.add_subplot(211)
    # ax2 = ax1.twinx()
    # for i in range(len(V_d_2[0 + int(repeat_interval_strongLFS*600 / step):int(1500/step) + int(repeat_interval_strongLFS*600 / step)])-1):
    #     if i >0 and V_d_2[i + int(repeat_interval_strongLFS*600 / step)] >= V_d_2[i + int(repeat_interval_strongLFS*600 / step) - 1]  \
    #             and V_d_2[i + int(repeat_interval_strongLFS*600 / step)] >= V_d_2[i + int(repeat_interval_strongLFS*600 / step)+1] and V_d_2[i + int(repeat_interval_strongLFS*600 / step)] >= -40:
    #         plt.axvline(x = (i*step-STC_1.calcium_time_window),ls = '--',c = 'y',linewidth = 1.0)
    # lns1 = ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,synapse_Ca_NMDA_strongLFS[0 + int(repeat_interval_strongLFS*600 / step):int(1500 / step) + int(repeat_interval_strongLFS*600 / step)],label=' synapse Ca NMDA-strong LFS', color="r")
    # lns2 = ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,synapse_Ca_NMDA_weakHFS[0 + int(repeat_interval_strongLFS*600 / step):int(1500 / step) + int(repeat_interval_strongLFS*600 / step)],label=' synapse Ca NMDA-weak HFS', color="b")
    # lns3 = ax2.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window, dend2_Ca_channel[0 + int(repeat_interval_strongLFS*600 / step):int(1500 / step) + int(repeat_interval_strongLFS*600 / step)], '--',label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca NMDA (μmol/L)", color="black")
    # ax2.set_ylabel('dend Ca channel (μmol/L)', color="black")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture6_strongLFS+weakHFS_3/1-Ca_average_strongLFS_2.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    # fig = plt.figure(figsize=(5.8, 4.0))
    # ax1 = fig.add_subplot(212)
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_average[0:1500] - STC_1.calcium_time_window, synapse_Ca_NMDA_strongLFS_average[0 + repeat_interval_strongLFS*800:1500 + repeat_interval_strongLFS*800],label=' synapse Ca NMDA-strong LFS', color="r")
    # lns2 = ax1.plot(t_average[0:1500] - STC_1.calcium_time_window, synapse_Ca_NMDA_weakHFS_average[0 + repeat_interval_strongLFS*800:1500 + repeat_interval_strongLFS*800],label='  synapse Ca NMDA-weak HFS', color="b")
    # lns3 = ax2.plot(t_average[0:1500] - STC_1.calcium_time_window, dend2_Ca_channel_average[0 + repeat_interval_strongLFS*800:1500 + repeat_interval_strongLFS*800], '--',label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca NMDA (μmol/L)", color="black")
    # ax2.set_ylabel('dend Ca channel (μmol/L)', color="black")
    ...
    # ax1 = fig.add_subplot(211)
    # ax2 = ax1.twinx()
    # for i in range(len(V_d_2[0 + int(repeat_interval_strongLFS*800 / step):int(1500/step) + int(repeat_interval_strongLFS*800 / step)])-1):
    #     if i >0 and V_d_2[i + int(repeat_interval_strongLFS*800 / step)] >= V_d_2[i + int(repeat_interval_strongLFS*800 / step) - 1]  and V_d_2[i + int(repeat_interval_strongLFS*800 / step)] >= V_d_2[i + int(repeat_interval_strongLFS*800 / step)+1] and V_d_2[i + int(repeat_interval_strongLFS*800 / step)] >= -40:
    #         plt.axvline(x = (i*step-STC_1.calcium_time_window),ls = '--',c = 'y',linewidth = 1.0)
    # lns1 = ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,synapse_Ca_NMDA_strongLFS[0 + int(repeat_interval_strongLFS*800 / step):int(1500 / step) + int(repeat_interval_strongLFS*800 / step)],label=' synapse Ca NMDA-strong LFS', color="r")
    # lns2 = ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,synapse_Ca_NMDA_weakHFS[0 + int(repeat_interval_strongLFS*800 / step):int(1500 / step) + int(repeat_interval_strongLFS*800 / step)],label=' synapse Ca NMDA-weak HFS', color="b")
    # lns3 = ax2.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window, dend2_Ca_channel[0 + int(repeat_interval_strongLFS*800 / step):int(1500 / step) + int(repeat_interval_strongLFS*800 / step)], '--',label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca NMDA (μmol/L)", color="black")
    # ax2.set_ylabel('dend Ca channel (μmol/L)', color="black")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture6_strongLFS+weakHFS_3/1-Ca_average_strongLFS_3.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    # fig = plt.figure(figsize=(5.8, 4.0))
    ...
    #                 synapse_Ca_NMDA_strongLFS_average[thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS):int(thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + 1500)],
    #                 label=' synapse Ca NMDA-strong LFS', color="r")
    # lns2 = ax1.plot(t_average[thirty_min:int(thirty_min + 1500)] - thirty_min - 500,
    #                 synapse_Ca_NMDA_weakHFS_average[thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS):int(thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + 1500)],
    #                 label='  synapse Ca NMDA-weak HFS', color="b")
    # lns3 = ax2.plot(t_average[thirty_min:int(thirty_min + 1500)] - thirty_min - 500,
    #                 dend2_Ca_channel_average[thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS):int(thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS) + 1500)], '--',
    ...
    #     if i > 0 and V_s[i + int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS)) / step)] >= V_s[i + int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS)) / step) - 1] and V_s[
    #         i + int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS)) / step)] >= V_s[i + int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS)) / step) + 1] and V_s[
    #         i + int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS)) / step)] >= -40:
    #         plt.axvline(x=(i * step - STC_1.calcium_time_window), ls='--', c='y', linewidth=1.0)
    # lns1 = ax1.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     synapse_Ca_NMDA_strongLFS[int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS)) / step):int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS)) / step) + int(1500 / step)],
    #     label=' synapse Ca NMDA-strong LFS', color="r")
    # lns2 = ax1.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     synapse_Ca_NMDA_weakHFS[int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS)) / step):int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS)) / step) + int(1500 / step)],
    #     label='  synapse Ca NMDA-weak HFS', color="b")
    # lns3 = ax2.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     dend2_Ca_channel[int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS)) / step):int((thirty_min + (stimu_Num_strongLFS) * (repeat_interval_strongLFS)) / step) + int(1500 / step)], '--',
    #     label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture6_strongLFS+weakHFS_3/1-Ca_average_weakHFS_1.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    ...
    # plt.savefig('cross-capture6_strongLFS+weakHFS_3/1-8.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    ...
    # plt.legend(loc = 1)
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture6_strongLFS+weakHFS_3/1-9.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(5, 5))
    # ax1 = fig.add_subplot(212)
    # plt.plot(t_neuron[0:int(1500 / step)] - 500, V_s[0 + int(repeat_interval_strongLFS/step):int(1500 / step) + int(repeat_interval_strongLFS/step)], label=' soma', color="blue")
    # plt.plot(t_neuron[0:int(1500 / step)] - 500, V_d_1[0 + int(repeat_interval_strongLFS/step):int(1500 / step) + int(repeat_interval_strongLFS/step)], "--", label=' dend1', color="red")
    # plt.plot(t_neuron[0:int(1500 / step)] - 500, V_d_2[0 + int(repeat_interval_strongLFS/step):int(1500 / step) + int(repeat_interval_strongLFS/step)], "--", label=' dend2', color="black")
    # ax1.set_ylabel("Voltage (mV)")
    # ax1.set_xlabel("time (ms)")
    # plt.title("postneuron spike train")
    # plt.xlim(0, 240)
    # plt.legend(loc = 1)
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture6_strongLFS+weakHFS_3/1-9_1.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(5, 5))
    # ax1 = fig.add_subplot(212)
    # plt.plot(t_neuron[0:int(1500 / step)] - 500, V_s[0 + int(repeat_interval_strongLFS*300/step):int(1500 / step) + int(repeat_interval_strongLFS*300/step)], label=' soma', color="blue")
    # plt.plot(t_neuron[0:int(1500 / step)] - 500, V_d_1[0 + int(repeat_interval_strongLFS*300/step):int(1500 / step) + int(repeat_interval_strongLFS*300/step)], "--", label=' dend1', color="red")
    # plt.plot(t_neuron[0:int(1500 / step)] - 500, V_d_2[0 + int(repeat_interval_strongLFS*300/step):int(1500 / step) + int(repeat_interval_strongLFS*300/step)], "--", label=' dend2', color="black")
    # ax1.set_ylabel("Voltage (mV)")
    # ax1.set_xlabel("time (ms)")
    # plt.title("postneuron spike train")
    # plt.xlim(0, 240)
    # plt.legend(loc = 1)
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture6_strongLFS+weakHFS_3/1-9_300.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    #
    # fig = plt.figure(figsize=(5, 5))
    # ax1 = fig.add_subplot(212)
    # plt.plot(t_neuron[0:int(1500 / step)] - 500, V_s[0 + int(repeat_interval_strongLFS*600/step):int(1500 / step) + int(repeat_interval_strongLFS*600/step)], label=' soma', color="blue")
    # plt.plot(t_neuron[0:int(1500 / step)] - 500, V_d_1[0 + int(repeat_interval_strongLFS*600/step):int(1500 / step) + int(repeat_interval_strongLFS*600/step)], "--", label=' dend1', color="red")
    # plt.plot(t_neuron[0:int(1500 / step)] - 500, V_d_2[0 + int(repeat_interval_strongLFS*600/step):int(1500 / step) + int(repeat_interval_strongLFS*600/step)], "--", label=' dend2', color="black")
    # ax1.set_ylabel("Voltage (mV)")
    # ax1.set_xlabel("time (ms)")
    # plt.title("postneuron spike train")
    # plt.xlim(0, 240)
    # plt.legend(loc = 1)
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture6_strongLFS+weakHFS_3/1-9_600.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    ...
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture6_strongLFS+weakHFS_3/1-15.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(6, 4.5))
    # ax1 = fig.add_subplot(211)
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,
    #                 synapse_Ca_NMDA_strongLFS[0 + int((repeat_interval_strongLFS)/step):int(1500 / step) + int((repeat_interval_strongLFS)/step)],
    #                 label=' synapse Ca NMDA-strong LFS', color="r")
    # lns2 = ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,
    #                 synapse_Ca_NMDA_weakHFS[0 + int((repeat_interval_strongLFS)/step):int(1500 / step) + int((repeat_interval_strongLFS)/step)],
    #                 label='  synapse Ca NMDA-weak HFS', color="b")
    # lns3 = ax2.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,
    #                 dend2_Ca_channel[0 + int((repeat_interval_strongLFS)/step):int(1500 / step) + int((repeat_interval_strongLFS)/step)], '--',
    #                 label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca_NMDA (μmol/L)", color="black")
    ...
    # plt.title("changes in calcium concentration(strong LFS)")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture6_strongLFS+weakHFS_3/1-15_1.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(6, 4.5))
    # ax1 = fig.add_subplot(211)
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,
    #                 synapse_Ca_NMDA_strongLFS[0 + int((repeat_interval_strongLFS*600)/step):int(1500 / step) + int((repeat_interval_strongLFS*600)/step)],
    #                 label=' synapse Ca NMDA-strong LFS', color="r")
    # lns2 = ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,
    #                 synapse_Ca_NMDA_weakHFS[0 + int((repeat_interval_strongLFS*600)/step):int(1500 / step) + int((repeat_interval_strongLFS*600)/step)],
    #                 label='  synapse Ca NMDA-weak HFS', color="b")
    # lns3 = ax2.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,
    #                 dend2_Ca_channel[0 + int((repeat_interval_strongLFS*600)/step):int(1500 / step) + int((repeat_interval_strongLFS*600)/step)], '--',
    #                 label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca_NMDA (μmol/L)", color="black")
    ...
    # plt.title("changes in calcium concentration(strong LFS)")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture6_strongLFS+weakHFS_3/1-15_2.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    #
    # fig = plt.figure(figsize=(6, 4.5))
    ...
    # plt.title("changes in calcium concentration")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture6_strongLFS+weakHFS_3/1-17.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # # fig = plt.figure(figsize=(10, 5))
    # # bax = brokenaxes(xlims=((-2, 70), (270, 302)), hspace=.05, despine=True)
    ...
    # # plt.savefig('cross-capture6_strongLFS+weakHFS_3/1-18.jpg')
    # # # plt.show()
    #
    # fig = plt.figure(figsize=(10, 5))
    # # membrane potention   ::  neuron with high CREB level
    ...
    # bax.set_xlabel("time (min)")
    # bax.set_title("postneuron firing rate")
    # plt.savefig('cross-capture6_strongLFS+weakHFS_3/1-19.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    #
    # # fig = plt.figure(figsize=(10, 5))
    ...
    # # bax.legend(loc=1)
    # # plt.title("changes in calcium concentration")
    # # plt.savefig('cross-capture6_strongLFS+weakHFS_3/1_7_1_1.jpg',bbox_inches = 'tight')
    # # plt.close()
    # plt.show()