...
class STC_model():
    def __init__(self):
        ...
        self.Ca0_synapse = 0.01  ## LTD tag
        self.Ca1_synapse = 0.2   ## LTP tag
        self.Ca0_dendritic = 0.025  ## PRP
...
if __name__ == '__main__':
    ...
    def calculation():
        ...
        for t_i in range(STC_1.calcium_time_window + 1, len(t_neuron)):
            number = int((t_i*step-STC_1.calcium_time_window)/ten_minute)
            # if (number in num) and (t_i*step-number*ten_minute-STC_1.calcium_time_window)<=repeat_interval*2:
            if ((number in num) and (t_i * step - number * ten_minute - STC_1.calcium_time_window) <= repeat_interval * 2):## 刺激消失后的2s后就不再计算V、I、Ca
                count = int((t_i * step - STC_1.calcium_time_window) / ten_minute)
                V_s[t_i], V_d_1[t_i], V_d_2[t_i],dend2_Ca_channel[t_i] = voltage.V_update(t_i, V_s[t_i - 1], V_d_1[t_i - 1], V_d_2[t_i - 1])
                synapse_Ca_NMDA[t_i], I_nmda_now = synapse.NMDA_Ca(t_i, synapse_Ca_NMDA[t_i - 1], V_d_2[t_i - 1], g_list_nmda, (np.array(preneuron_spikes_time[number]) + ten_minute * count + STC_1.calcium_time_window).tolist())
                I_ampa_now = synapse.I_AMPA(t_i, V_d_2[t_i], g_list_ampa, (np.array(preneuron_spikes_time[number]) + ten_minute * count + STC_1.calcium_time_window).tolist(), z_synapse[int(np.ceil((t_i * step) / STC_1.tag_prp_check_window))-1])
            ...
            if t_i * step>1000 and np.mod(t_i * step - STC_1.calcium_time_window, 1000) == 0.0:
                ...
                f = open("100Hz_10/firing rate.txt", "a")
    ...
    resultH5 = h5py.File('100Hz_10.hdf5', 'w')
    resultH5.create_dataset(name='V_s', data=(np.array(V_s[0:int(((stimu_times-1) * ten_minute + int(STC_1.calcium_time_window)*2 + time_spike_train_one)/step)])).tolist())
    resultH5.create_dataset(name='V_d_1', data=(np.array(V_d_1[0:int(((stimu_times-1) * ten_minute + int(STC_1.calcium_time_window)*2 + time_spike_train_one)/step)])).tolist())
    resultH5.create_dataset(name='V_d_2', data=(np.array(V_d_2[0:int(((stimu_times-1) * ten_minute + int(STC_1.calcium_time_window)*2 + time_spike_train_one)/step)])).tolist())
    resultH5.create_dataset(name='soma_Ca_channel', data=(np.array(soma_Ca_channel[0:int(((stimu_times-1) * ten_minute + int(STC_1.calcium_time_window)*2 + time_spike_train_one)/step)])).tolist())
    resultH5.create_dataset(name='dend_Ca_channel', data=(np.array(dend2_Ca_channel[0:int(((stimu_times-1) * ten_minute + int(STC_1.calcium_time_window)*2 + time_spike_train_one)/step)])).tolist())
    resultH5.create_dataset(name='synapse_Ca_NMDA', data=(np.array(synapse_Ca_NMDA[0:int(((stimu_times-1) * ten_minute + int(STC_1.calcium_time_window)*2 + time_spike_train_one)/step)])).tolist())
    ...
    # plt.savefig('100Hz_10/1.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    ...
    # plt.tight_layout(0.1)
    # plt.savefig('100Hz_10/2.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    #
    # fig = plt.figure(figsize=(10, 3.5))
    ...
    # plt.title("synaptic weight changes : LTP")
    # plt.tight_layout(0.1)
    # plt.savefig('100Hz_10/1_1.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    #
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('100Hz_10/6_add1.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    # soma_Ca_channel_average = np.zeros(5 * 60 * 60 * 1000)  ##5h  1ms为单位
    ...
    # synapse_Ca_NMDA_average = np.zeros(5 * 60 * 60 * 1000)
    #
    # for i in range(len(synapse_Ca_NMDA_average[0:1500])):
    #     dend2_Ca_channel_average[i + 20 * 60 * 1000] = sum(dend2_Ca_channel[int((i + 20 * 60 * 1000 - STC_1.tag_prp_check_window) / step):int((i + 20 * 60 * 1000) / step)]) / int(STC_1.tag_prp_check_window / step)
    #     synapse_Ca_NMDA_average[i + 20 * 60 * 1000] = sum(synapse_Ca_NMDA[int((i + 20 * 60 * 1000 - STC_1.tag_prp_check_window) / step):int((i + 20 * 60 * 1000) / step)]) / int(STC_1.tag_prp_check_window / step)
    #
    # # t_average = np.arange(0, len(synapse_Ca_NMDA_average), 1)
    # fig = plt.figure(figsize=(6, 4.5))
    # ax1 = fig.add_subplot(212)
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_average[0 + 20 * 60 * 1000:1500 + 20 * 60 * 1000] - 500 - 20 * 60 * 1000,
    #                 synapse_Ca_NMDA_average[0 + 20 * 60 * 1000:1500 + 20 * 60 * 1000],
    #                 label=' synapse Ca NMDA', color="r")
    # lns3 = ax2.plot(t_average[0 + 20 * 60 * 1000:1500 + 20 * 60 * 1000] - 500 - 20 * 60 * 1000,
    #                 dend2_Ca_channel_average[0 + 20 * 60 * 1000:1500 + 20 * 60 * 1000], '--',
    ...
    # ax2 = ax1.twinx()
    # # ax3 = ax1.twinx()
    # for i in range(len(V_s[0 + int(20 * 60 * 1000 / step):int(1500 / step)]) + int(20 * 60 * 1000 / step) - 1):
    #     if V_s[i + int(20 * 60 * 1000 / step)] >= V_s[i + int(20 * 60 * 1000 / step) - 1] and V_s[
    #         i + int(20 * 60 * 1000 / step)] >= V_s[i + int(20 * 60 * 1000 / step) + 1] and V_s[
    #         i + int(20 * 60 * 1000 / step)] >= -20:
    #         plt.axvline(x=(i * step -  STC_1.calcium_time_window), ls='--', c='y', linewidth=1.0)
    # ax1.plot(
    #     t_neuron[0 + int(20 * 60 * 1000 / step):int(1500 / step) + int(
    #         20 * 60 * 1000 / step)] -  STC_1.calcium_time_window - 20 * 60 * 1000,
    #     synapse_Ca_NMDA[0 + int(20 * 60 * 1000 / step):int(1500 / step) + int(20 * 60 * 1000 / step)],
    #     label=' synapse Ca NMDA', color="r")
    # ax2.plot(
    #     t_neuron[0 + int(20 * 60 * 1000 / step):int(1500 / step) + int(
    #         20 * 60 * 1000 / step)] - STC_1.calcium_time_window - 20 * 60 * 1000,
    #     dend2_Ca_channel[0 + int(20 * 60 * 1000 / step):int(1500 / step) + int(20 * 60 * 1000 / step)], '--',
    #     label=' dend Ca channel', color="black")
    # # ax3.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window, V_d_2[0:int(1500 / step)], '--',
    # #                 label=' dend2 spike time', color="y")
    # plt.xlim(0, 1000)
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('100Hz_10/6_add2.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    # fig = plt.figure(figsize=(10, 5))
    ...
    # plt.savefig('100Hz_10/4_1.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    #
    ...
    # ax1.legend(loc=1)
    # plt.tight_layout(0.1)
    # plt.savefig('100Hz_10/5.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(5, 5))
    # ax1 = fig.add_subplot(212)
    ...
    # ax1.legend(loc=1)
    # plt.tight_layout(0.1)
    # plt.savefig('100Hz_10/5_1_2.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    #
    # fig = plt.figure(figsize=(5, 5))
    # ax1 = fig.add_subplot(212)
    # plt.plot(t_neuron[int(1200000 / step):int((1200000 + 1500) / step)] - STC_1.calcium_time_window-1200000, V_s[int(1200000 / step):int((1200000 + 1500) / step)], label=' soma',
    #          color="blue")
    # plt.plot(t_neuron[int(1200000 / step):int((1200000 + 1500) / step)] - STC_1.calcium_time_window-1200000, V_d_1[int(1200000 / step):int((1200000 + 1500) / step)], "--", label=' dend1',
    #          color="red")
    # plt.plot(t_neuron[int(1200000 / step):int((1200000 + 1500) / step)] - STC_1.calcium_time_window-1200000, V_d_2[int(1200000 / step):int((1200000 + 1500) / step)], "--", label=' dend2',
    #          color="black")
    # ax1.set_ylabel("Voltage (mV)")
    # ax1.set_xlabel("time (ms)")
    # plt.title("postneuron spike train ")
    ...
    # ax1.legend(loc=1)
    # plt.tight_layout(0.1)
    # plt.savefig('100Hz_10/5_1.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(5, 5))
    # ax1 = fig.add_subplot(212)
    ...
    # ax1.legend(loc=1)
    # plt.tight_layout(0.1)
    # plt.savefig('100Hz_10/5_1_1.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    #
    ...
    # plt.savefig('100Hz_10/6.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # # fig = plt.figure(figsize=(10, 5))
    ...
    # # bax.legend(loc=1)
    # # plt.title("changes in calcium concentration")
    # # plt.savefig('100Hz_10/7_1_1.jpg',bbox_inches = 'tight')
    # # plt.close()
    # # plt.show()
    #
    #
