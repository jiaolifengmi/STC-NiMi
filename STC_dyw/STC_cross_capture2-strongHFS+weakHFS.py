...
if __name__ == '__main__':
    ...
    time_spike_train_one_strongHFS = 1000  ## ms  一次刺激时长  200
    ...
    preneuron_spikes_train_weakHFS, preneuron_spikes_time_weakHFS = Poisson_spike_100Hz.poisson_spike_train_H(trail_No-1,stimu_Num_weakHFS,
                                                                                                              time_spike_train_one_weakHFS,
                                                                                                              preneuron_firing_rate_weakHFS,
                                                                                                              repeat_times,repeat_interval)
    preneuron_firing_rate_strongHFS = 100  ##Hz
    preneuron_spikes_train_strongHFS, preneuron_spikes_time_strongHFS = Poisson_spike_100Hz.poisson_spike_train_H(trail_No-1,stimu_Num_strongHFS,
                                                                                                                  time_spike_train_one_strongHFS,
                                                                                                                  preneuron_firing_rate_strongHFS,
                                                                                                                  repeat_times,repeat_interval)
    ...
    def calculation():
        ...
        for t_i in range(STC_2.calcium_time_window+1,len(t_neuron)):
            num_strong = int((t_i*step-STC_2.calcium_time_window)/ten_min)
            if num_strong in np.arange(0, stimu_Num_strongHFS).tolist() \
                    and (t_i * step - num_strong * ten_min - STC_2.calcium_time_window) <= repeat_interval * 2:
                time_ = 1
                count = int((t_i * step - STC_2.calcium_time_window) / ten_min)  ##0-n次刺激,repeat_interval_strongHFS = 10min/1次
                ...
                synapse_Ca_NMDA_strongHFS[t_i],I_nmda_strongHFS_now = synapse_strongHFS.NMDA_Ca(t_i,synapse_Ca_NMDA_strongHFS[t_i - 1],V_d_2[t_i - 1],g_list_nmda_strongHFS,(np.array(preneuron_spikes_time_strongHFS[count]) + ten_min * count + STC_2.calcium_time_window).tolist())
                I_ampa_strongHFS_now = synapse_strongHFS.I_AMPA(t_i,V_d_2[t_i],g_list_ampa_strongHFS,(np.array(preneuron_spikes_time_strongHFS[int(count)]) + ten_min * count +STC_2.calcium_time_window).tolist(),z_synapse_2[int(np.ceil((t_i*step) / STC_2.tag_prp_check_window))-1])
            ...
            ##30min 后给另一个刺激
            elif t_i * step < (thirty_min + (stimu_Num_strongHFS-1) * (repeat_interval_strongHFS) + time_spike_train_one_strongHFS + STC_1.calcium_time_window + (stimu_Num_weakHFS) * (repeat_interval_weakHFS) )\
                ...
                time_ = 2
                count = int((t_i * step - STC_1.calcium_time_window  - thirty_min - (stimu_Num_strongHFS-1) * (repeat_interval_strongHFS) - time_spike_train_one_strongHFS) / repeat_interval_weakHFS)  ##0-n次刺激,repeat_interval = 1000ms/1次
                ...
                ### weakHFS
                V_s[t_i],V_d_1[t_i],V_d_2[t_i],dend2_Ca_channel[t_i] = voltage.V_update(t_i,V_s[t_i-1],V_d_1[t_i-1],V_d_2[t_i-1])
                synapse_Ca_NMDA_weakHFS[t_i],I_nmda_weakHFS_now = synapse_weakHFS.NMDA_Ca(t_i,synapse_Ca_NMDA_weakHFS[t_i - 1],V_d_2[t_i - 1],g_list_nmda_weakHFS,(np.array(preneuron_spikes_time_weakHFS[int(count)])+ thirty_min +  + STC_1.calcium_time_window + repeat_interval * count + time_spike_train_one_strongHFS + (stimu_Num_strongHFS-1) * (repeat_interval_strongHFS)).tolist())
                I_ampa_weakHFS_now = synapse_weakHFS.I_AMPA(t_i,V_d_2[t_i],g_list_ampa_weakHFS,(np.array(preneuron_spikes_time_weakHFS[int(count)]) + thirty_min + STC_1.calcium_time_window + repeat_interval * count + time_spike_train_one_strongHFS + (stimu_Num_strongHFS-1) * (repeat_interval_strongHFS)).tolist(),z_synapse_1[int(np.ceil((t_i*step) / STC_1.tag_prp_check_window))-1])
                voltage.I_syn_d2 = I_nmda_weakHFS_now + I_ampa_weakHFS_now + I_nmda_strongHFS_now + I_ampa_strongHFS_now
            ...
            if t_i * step>1000 and np.mod(t_i * step - STC_1.calcium_time_window, 1000) == 0.0:
                ...
                f = open("cross-capture2_strongHFS+weakHFS_5/firing rate.txt", "a")
        ...
        f = open("cross-capture2_strongHFS+weakHFS_5/运行时间.txt", "a")
        f.write("Biological time : " + str(time_neuron / 1000) + "s" + "   calculating time : " + str(round(toc - tic, 3)) + "s")
        f.write("\n")
        f.close()
    ...
    resultH5 = h5py.File('cross-capture2_strongHFS+weakHFS_5.hdf5', 'w')
    resultH5.create_dataset(name='V_s', data=(np.array(V_s[0:int((STC_1.calcium_time_window*2 + (stimu_Num_strongHFS - 1) *repeat_interval_strongHFS + time_spike_train_one_strongHFS + thirty_min +  stimu_Num_weakHFS * repeat_interval_weakHFS)/step)])).tolist())
    resultH5.create_dataset(name='V_d_1', data=(np.array(V_d_1[0:int((STC_1.calcium_time_window*2 + (stimu_Num_strongHFS - 1) *repeat_interval_strongHFS + time_spike_train_one_strongHFS + thirty_min +  stimu_Num_weakHFS * repeat_interval_weakHFS)/step)])).tolist())
    resultH5.create_dataset(name='V_d_2', data=(np.array(V_d_2[0:int((STC_1.calcium_time_window*2 + (stimu_Num_strongHFS - 1) *repeat_interval_strongHFS + time_spike_train_one_strongHFS + thirty_min +  stimu_Num_weakHFS * repeat_interval_weakHFS)/step)])).tolist())
    resultH5.create_dataset(name='soma_Ca_channel', data=(np.array(soma_Ca_channel[0:int((STC_1.calcium_time_window*2 + (stimu_Num_strongHFS - 1) *repeat_interval_strongHFS + time_spike_train_one_strongHFS + thirty_min +  stimu_Num_weakHFS * repeat_interval_weakHFS)/step)])).tolist())
    resultH5.create_dataset(name='dend_Ca_channel', data=(np.array(dend2_Ca_channel[0:int((STC_1.calcium_time_window*2 + (stimu_Num_strongHFS - 1) *repeat_interval_strongHFS + time_spike_train_one_strongHFS + thirty_min +  stimu_Num_weakHFS * repeat_interval_weakHFS)/step)])).tolist())
    resultH5.create_dataset(name='synapse_Ca_NMDA_strongHFS', data=(np.array(synapse_Ca_NMDA_strongHFS[0:int((STC_1.calcium_time_window*2 + (stimu_Num_strongHFS - 1) *repeat_interval_strongHFS + time_spike_train_one_strongHFS + thirty_min +  stimu_Num_weakHFS * repeat_interval_weakHFS)/step)])).tolist())
    resultH5.create_dataset(name='synapse_Ca_NMDA_weakHFS', data=(np.array(synapse_Ca_NMDA_weakHFS[0:int((STC_1.calcium_time_window*2 + (stimu_Num_strongHFS - 1) *repeat_interval_strongHFS + time_spike_train_one_strongHFS + thirty_min +  stimu_Num_weakHFS * repeat_interval_weakHFS)/step)])).tolist())
    # resultH5.create_dataset(name='dend_Ca_channel_sub', data=(np.array(dend2_Ca_channel_sub),2).tolist())
    # resultH5.create_dataset(name='synapse_Ca_NMDA_strongHFS_sub', data=(np.array(synapse_Ca_NMDA_strongHFS_sub),2).tolist())
    # resultH5.create_dataset(name='synapse_Ca_NMDA_weakHFS_sub', data=(np.array(synapse_Ca_NMDA_weakHFS_sub),2).tolist())
    resultH5.create_dataset(name='z_synapse_strongHFS', data=(np.array(z_synapse_2)).tolist())
    resultH5.create_dataset(name='z_synapse_weakHFS', data=(np.array(z_synapse_1)).tolist())
    resultH5.create_dataset(name='prp', data=(np.array(prp)).tolist())
    resultH5.create_dataset(name='tag_synapse_strongHFS', data=(np.array(tag_synapse_2)).tolist())
    resultH5.create_dataset(name='tag_synapse_weakHFS', data=(np.array(tag_synapse_1)).tolist())
    resultH5.create_dataset(name='y_synapse_strongHFS', data=(np.array(y_synapse_2)).tolist())
    resultH5.create_dataset(name='y_synapse_weakHFS', data=(np.array(y_synapse_1)).tolist())
    resultH5.create_dataset(name='tag_flag_strongHFS', data=(np.array(tag_flag_synapse_2)).tolist())
    resultH5.create_dataset(name='tag_flag_weakHFS', data=(np.array(tag_flag_synapse_1)).tolist())
    ...
    # plt.savefig('cross-capture2_strongHFS+weakHFS_5/1-1_prp1000.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # # # plt.show()
    #
    ...
    # time_scale_tag_prp = (1000 / STC_1.tag_prp_check_window) * 60
    # ax4 = ax3.twinx()
    # lns66 = ax3.plot(tag_prp_time / time_scale_tag_prp, z_synapse_2,label='z synapse(strong HFS)',color='black')  ##familiar
    # lns6 = ax3.plot(tag_prp_time / time_scale_tag_prp, z_synapse_1, '--',  label='z synapse(weak HFS)',color='black')
    # lns33 = ax4.plot(tag_prp_time / time_scale_tag_prp, tag_synapse_2, label='tag synapse(strong HFS)',color='red')
    # lns3 = ax4.plot(tag_prp_time / time_scale_tag_prp, tag_synapse_1, '--', label='tag synapse(weak HFS)', color='red')
    # lns4 = ax4.plot(tag_prp_time / time_scale_tag_prp, prp*1000, label='PRP', color='b')
    ...
    # plt.title(" synaptic weight changes")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture2_strongHFS+weakHFS_5/1-1_1_prp1000.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    #
    #
    ...
    # plt.title(" synaptic weight changes")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture2_strongHFS+weakHFS_5/1-1_1_prp100.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    #
    ...
    # # plt.tight_layout(0.1)
    # # plt.savefig('cross-capture2/1-7.jpg')
    # # # # plt.show()
    #
    # step = 0.05
    ...
    #
    # for i in range(STC_1.calcium_time_window + (stimu_Num_strongHFS-1) * (repeat_interval_strongHFS) + time_spike_train_one_strongHFS + thirty_min + stimu_Num_weakHFS*repeat_interval_weakHFS + 1500):
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture2_strongHFS+weakHFS_5/1-Ca_average_strongHFS_1.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    #
    ...
    # ax1 = fig.add_subplot(212)
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_average[0:1500] - STC_1.calcium_time_window, synapse_Ca_NMDA_strongHFS_average[0 + ten_min:1500 + ten_min],label=' synapse Ca NMDA-strong HFS', color="r")
    # lns2 = ax1.plot(t_average[0:1500] - STC_1.calcium_time_window, synapse_Ca_NMDA_weakHFS_average[0 + ten_min:1500 + ten_min],label='  synapse Ca NMDA-weak HFS', color="b")
    # lns3 = ax2.plot(t_average[0:1500] - STC_1.calcium_time_window, dend2_Ca_channel_average[0 + ten_min:1500 + ten_min], '--',label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca NMDA (μmol/L)", color="black")
    # ax2.set_ylabel('dend Ca channel (μmol/L)', color="black")
    ...
    # lns2 = ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,synapse_Ca_NMDA_weakHFS[0 + int(ten_min / step):int(1500 / step) + int(ten_min / step)],label=' synapse Ca NMDA-weak HFS', color="b")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture2_strongHFS+weakHFS_5/1-Ca_average_strongHFS_2.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    # fig = plt.figure(figsize=(5.8, 4.0))
    ...
    # lns2 = ax1.plot(t_average[0:1500] - STC_1.calcium_time_window, synapse_Ca_NMDA_weakHFS_average[0 + ten_min*2:1500 + ten_min*2],label='  synapse Ca NMDA-weak HFS', color="b")
    ...
    # lns2 = ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window,synapse_Ca_NMDA_weakHFS[0 + int(ten_min*2 / step):int(1500 / step) + int(ten_min*2 / step)],label=' synapse Ca NMDA-weak HFS', color="b")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture2_strongHFS+weakHFS_5/1-Ca_average_strongHFS_3.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    # fig = plt.figure(figsize=(5.8, 4.0))
    ...
    #                 synapse_Ca_NMDA_weakHFS_average[thirty_min + (stimu_Num_strongHFS-1) * (repeat_interval_strongHFS) + time_spike_train_one_strongHFS:int(thirty_min + (stimu_Num_strongHFS-1) * (repeat_interval_strongHFS)  + time_spike_train_one_strongHFS+ 1500)],
    ...
    #     synapse_Ca_NMDA_strongHFS[int((thirty_min + (stimu_Num_strongHFS-1) * (repeat_interval_strongHFS) + time_spike_train_one_strongHFS) / step)
    #                               :int((thirty_min + (stimu_Num_strongHFS-1) * (repeat_interval_strongHFS) + time_spike_train_one_strongHFS) / step) + int(1500 / step)],
    #     label=' synapse Ca NMDA-strong HFS', color="r")
    # lns2 = ax1.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     synapse_Ca_NMDA_weakHFS[int((thirty_min + (stimu_Num_strongHFS-1) * (repeat_interval_strongHFS) + time_spike_train_one_strongHFS) / step)
    #                             :int((thirty_min + (stimu_Num_strongHFS-1) * (repeat_interval_strongHFS) + time_spike_train_one_strongHFS) / step) + int(1500 / step)],
    #     label='  synapse Ca NMDA-weak HFS', color="b")
    # lns3 = ax2.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     dend2_Ca_channel[int((thirty_min + (stimu_Num_strongHFS-1) * (repeat_interval_strongHFS) + time_spike_train_one_strongHFS) / step)
    #                      :int((thirty_min + (stimu_Num_strongHFS-1) * (repeat_interval_strongHFS) + time_spike_train_one_strongHFS) / step) + int(1500 / step)], '--',
    #     label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture2_strongHFS+weakHFS_5/1-Ca_average_weakHFS_1.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    #
    ...
    # plt.savefig('cross-capture2_strongHFS+weakHFS_5/1-8.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    ...
    # plt.legend(loc = 1)
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture2_strongHFS+weakHFS_5/1-9.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(5, 5))
    # ax1 = fig.add_subplot(212)
    # plt.plot(t_neuron[0:int(1500 / step)] - 500,
    #          V_s[0 + int(repeat_interval_strongHFS / step):int(1500 / step) + int(repeat_interval_strongHFS / step)], label=' soma',
    #          color="blue")
    # plt.plot(t_neuron[0:int(1500 / step)] - 500,
    #          V_d_1[0 + int(repeat_interval_strongHFS / step):int(1500 / step) + int(repeat_interval_strongHFS / step)], "--",
    #          label=' dend1', color="red")
    # plt.plot(t_neuron[0:int(1500 / step)] - 500,
    #          V_d_2[0 + int(repeat_interval_strongHFS / step):int(1500 / step) + int(repeat_interval_strongHFS / step)], "--",
    #          label=' dend2', color="black")
    # ax1.set_ylabel("Voltage (mV)")
    # ax1.set_xlabel("time (ms)")
    # plt.title("postneuron spike train")
    # plt.xlim(0, 1000)
    # plt.legend(loc=1)
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture2_strongHFS+weakHFS_5/1-9_1.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(5, 5))
    # ax1 = fig.add_subplot(212)
    # plt.plot(t_neuron[0:int(1500 / step)] - 500,
    #          V_s[0 + int(repeat_interval_strongHFS * 2 / step):int(1500 / step) + int(repeat_interval_strongHFS * 2 / step)],
    #          label=' soma', color="blue")
    # plt.plot(t_neuron[0:int(1500 / step)] - 500,
    #          V_d_1[0 + int(repeat_interval_strongHFS * 2/step):int(1500 / step) + int(repeat_interval_strongHFS * 2 / step)], "--",
    #          label=' dend1', color="red")
    # plt.plot(t_neuron[0:int(1500 / step)] - 500,
    #          V_d_2[0 + int(repeat_interval_strongHFS * 2 / step):int(1500 / step) + int(repeat_interval_strongHFS * 2 / step)], "--",
    #          label=' dend2', color="black")
    # ax1.set_ylabel("Voltage (mV)")
    # ax1.set_xlabel("time (ms)")
    # plt.title("postneuron spike train")
    ...
    # plt.legend(loc=1)
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture2_strongHFS+weakHFS_5/1-9_2.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(5, 5))
    # ax1 = fig.add_subplot(212)
    # plt.plot(t_neuron[0:int(1500 / step)] - 500, V_s[0 + int(((stimu_Num_strongHFS-1)*repeat_interval_strongHFS + time_spike_train_one_strongHFS + thirty_min)/step):int(1500 / step) + int(((stimu_Num_strongHFS-1)*repeat_interval_strongHFS + time_spike_train_one_strongHFS + thirty_min)/step)], label=' soma', color="blue")
    # plt.plot(t_neuron[0:int(1500 / step)] - 500, V_d_1[0 + int(((stimu_Num_strongHFS-1)*repeat_interval_strongHFS + time_spike_train_one_strongHFS + thirty_min)/step):int(1500 / step) + int(((stimu_Num_strongHFS-1)*repeat_interval_strongHFS + time_spike_train_one_strongHFS + thirty_min)/step)], "--", label=' dend1', color="red")
    # plt.plot(t_neuron[0:int(1500 / step)] - 500, V_d_2[0 + int(((stimu_Num_strongHFS-1)*repeat_interval_strongHFS + time_spike_train_one_strongHFS + thirty_min)/step):int(1500 / step) + int(((stimu_Num_strongHFS-1)*repeat_interval_strongHFS + time_spike_train_one_strongHFS + thirty_min)/step)], "--", label=' dend2', color="black")
    # ax1.set_ylabel("Voltage (mV)")
    # ax1.set_xlabel("time (ms)")
    # plt.title("postneuron spike train")
    # plt.xlim(0, 1000)
    # plt.legend(loc = 1)
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture2_strongHFS+weakHFS_5/1-9_3.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    ...
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture2_strongHFS+weakHFS_5/1-15.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(6, 4.5))
    # ax1 = fig.add_subplot(212)
    ...
    # lns2 = ax1.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    #     synapse_Ca_NMDA_weakHFS[int((thirty_min + (stimu_Num_strongHFS-1) * (repeat_interval_strongHFS) + time_spike_train_one_strongHFS) / step):int((thirty_min + (stimu_Num_strongHFS-1) * (repeat_interval_strongHFS) + time_spike_train_one_strongHFS) / step) + int(1500 / step)],
    #     label='  synapse Ca NMDA-weak HFS', color="b")
    # lns3 = ax2.plot(
    #     t_neuron[int(thirty_min / step):int(thirty_min / step) + int(1500 / step)] - thirty_min - STC_1.calcium_time_window,
    ...
    #     label=' 240 Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca NMDA (μmol/L)", color="black")
    # ax2.set_ylabel('dend Ca channel (μmol/L)', color="black")
    # plt.title("changes in calcium concentration(weak HFS)")
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture2_strongHFS+weakHFS_5/1-16_1.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    #
    ...
    # plt.tight_layout(0.1)
    # plt.savefig('cross-capture2_strongHFS+weakHFS_5/1-17.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # # fig = plt.figure(figsize=(10, 5))
    # # bax = brokenaxes(xlims=((-2, 70), (270, 302)), hspace=.05, despine=True)
    ...
    # # plt.savefig('cross-capture2_strongHFS+weakHFS_5/1-18.jpg')
    # # # plt.show()
    #
    # fig = plt.figure(figsize=(10, 5))
    # # membrane potention   ::  neuron with high CREB level
    ...
    # bax.set_xlabel("time (min)")
    # bax.set_title("postneuron firing rate")
    # plt.savefig('cross-capture2_strongHFS+weakHFS_5/1-19.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    #
    # # fig = plt.figure(figsize=(10, 5))
    ...
    # # bax.legend(loc=1)
    # # plt.title("changes in calcium concentration")
    # # plt.savefig('cross-capture2_strongHFS+weakHFS_5/1_7_1_1.jpg',bbox_inches = 'tight')
    # # plt.close()
    # # plt.show()