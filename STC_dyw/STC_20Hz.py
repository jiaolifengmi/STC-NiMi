...
if __name__ == '__main__':
    ...
    ## 30Hz刺激  200ms/1s * 900
    time_spike_train_one = 150  ## ms  一次刺激时长  200
    preneuron_firing_rate_L = 20  ##Hz
    repeat_times = 1  ##这个参数没什么用了
    repeat_interval = 1000  # ms
    stimu_times = 900   ##刺激重复呈现的次数
    trail_No = 10  ##第几次实验 1-10(for calulate bar)
    preneuron_spikes_train, preneuron_spikes_time = Poisson_spike_low_Frequency.poisson_spike_train_L(trail_No-1,stimu_times,time_spike_train_one, preneuron_firing_rate_L, repeat_times, repeat_interval)
    ...
    def calculation():
        ...
        for t_i in range(STC_1.calcium_time_window + 1, len(t_neuron)):
            if t_i * step < stimu_times * (repeat_interval) + STC_1.calcium_time_window*2 :
                count = int((t_i * step - STC_1.calcium_time_window) / repeat_interval)  ##0-899次刺激,1000ms/1次
                if t_i * step < STC_1.calcium_time_window + stimu_times * stimu_times+repeat_interval:
                    ...
                    synapse_Ca_NMDA[t_i], I_nmda_now = synapse.NMDA_Ca(t_i, synapse_Ca_NMDA[t_i - 1], V_d_2[t_i - 1], g_list_nmda, (np.array(preneuron_spikes_time[count]) + repeat_interval * count + int(STC_1.calcium_time_window)).tolist())
                    I_ampa_now = synapse.I_AMPA(t_i, V_d_2[t_i], g_list_ampa, (np.array(preneuron_spikes_time[count]) + repeat_interval * count + int(STC_1.calcium_time_window)).tolist(), z_synapse[int(np.ceil((t_i * step) / STC_1.tag_prp_check_window))-1])
            ...
            if V_s[t_i-1] >= V_s[t_i-2] and V_s[t_i-1]  >= 0 and V_s[t_i-2] <= 0 and (((t_i - 1) * step - spike_time_pre) >= 5):  ##给刺激的1000ms
                # V_s[i] >= V_s[i - 1] and V_s[i] >= V_s[i + 1] and V_s[i] >= -20
            ...
            if np.mod(t_i * step, STC_1.tag_prp_check_window) == 0.0:  # tag——prp时间窗长度,更新Tag/PRP/y/z
                ...
                print('syn、dend、z_synapse')
                print(synapse_Ca_NMDA_average[int(t_i*step)])
                print(dend2_Ca_channel_average[int(t_i*step)])
                print(z_synapse[int(np.ceil((t_i * step) / STC_1.tag_prp_check_window))-1])
                print('---------------------------')
    ...
    resultH5 = h5py.File('20Hz_10.hdf5', 'w')
    resultH5.create_dataset(name='V_s', data=np.array(
        V_s[0:int((stimu_times * repeat_interval + int(STC_1.calcium_time_window) * 2) / step)]).tolist())
    resultH5.create_dataset(name='V_d_1', data=np.array(
        V_d_1[0:int((stimu_times * repeat_interval + int(STC_1.calcium_time_window) * 2) / step)]).tolist())
    resultH5.create_dataset(name='V_d_2', data=np.array(
        V_d_2[0:int((stimu_times * repeat_interval + int(STC_1.calcium_time_window) * 2) / step)]).tolist())
    resultH5.create_dataset(name='soma_Ca_channel', data=np.array(
        soma_Ca_channel[0:int((stimu_times * repeat_interval + int(STC_1.calcium_time_window) * 2) / step)]).tolist())
    resultH5.create_dataset(name='dend_Ca_channel', data=np.array(
        dend2_Ca_channel[0:int((stimu_times * repeat_interval + int(STC_1.calcium_time_window) * 2) / step)]).tolist())
    resultH5.create_dataset(name='synapse_Ca_NMDA', data=np.array(
        synapse_Ca_NMDA[0:int((stimu_times * repeat_interval + int(STC_1.calcium_time_window) * 2) / step)]).tolist())
    # resultH5.create_dataset(name='dend_Ca_channel_sub', data=np.round(np.array(dend2_Ca_channel_sub),1).tolist())
    # resultH5.create_dataset(name='synapse_Ca_NMDA_sub', data=np.round(np.array(synapse_Ca_NMDA_sub),1).tolist())
    ...
    # resultH5.create_dataset(name='dend2_Ca_channel_average', data=np.round(np.array(dend2_Ca_channel_average),1).tolist())
    # resultH5.create_dataset(name='synapse_Ca_NMDA_average', data=np.round(np.array(synapse_Ca_NMDA_average),1).tolist())
    resultH5.close()
    ...
    # lns1 = ax1.plot(tag_prp_time / time_scale_tag_prp, prp_x1, label='prp_x1')
    # lns2 = ax1.plot(tag_prp_time / time_scale_tag_prp, prp_x2, label='prp_x2')
    # ax1.set_xlabel("Time (min)")
    # ax1.set_ylabel('prp rate', color='black')
    # lns = lns1 + lns2
    # labs = [l.get_label() for l in lns]
    # ax1.legend(lns, labs, loc=1)
    # # plt.title("synaptic weight changes : LTD")
    # plt.tight_layout(0.1)
    # plt.savefig('20Hz_10/1.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    # # Visualization
    ...
    # ax2 = ax1.twinx()
    # time_scale_tag_prp = (1000 / STC_1.tag_prp_check_window) * 60
    # lns1 = ax1.plot(tag_prp_time[0:-16] / time_scale_tag_prp, z_synapse[0:-16], label='z_synapse',color='black')
    # lns2 = ax2.plot(tag_prp_time[0:-16] / time_scale_tag_prp, tag_synapse[0:-16], label='tag_synapse', color='red')
    # lns3 = ax2.plot(tag_prp_time[0:-16] / time_scale_tag_prp, prp[0:-16]*1000, label='PRP', color='b')
    # lns4 = ax2.plot(tag_prp_time[0:-16] / time_scale_tag_prp, y_synapse[0:-16], label='y_synapse', color='y')
    # ax1.set_xlabel("Time (min)")
    # ax1.set_ylabel('z_synapse', color='black')
    # ax2.set_ylabel("Amount(tag PRP y)")
    ...
    # plt.title("synaptic weight changes : LTD")
    # plt.tight_layout(0.1)
    # plt.savefig('20Hz_10/2_prp1000.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    # # Visualization
    ...
    # plt.title("synaptic weight changes : LTD")
    # plt.tight_layout(0.1)
    # plt.savefig('20Hz_10/2_1_prp1000.jpg', bbox_inches='tight')
    # plt.close()
    # # plt.show()
    #
    # # Visualization
    ...
    # lns1 = ax1.plot(tag_prp_time[0:-16] / time_scale_tag_prp, z_synapse[0:-16], label='z_synapse',color='black')
    # lns2 = ax2.plot(tag_prp_time[0:-16] / time_scale_tag_prp, tag_synapse[0:-16], label='tag_synapse', color='red')
    # lns3 = ax2.plot(tag_prp_time[0:-16] / time_scale_tag_prp, prp[0:-16]*100, label='PRP', color='b')
    # lns4 = ax2.plot(tag_prp_time[0:-16] / time_scale_tag_prp, y_synapse[0:-16]/100, label='y_synapse', color='y')
    # ax1.set_xlabel("Time (min)")
    # ax1.set_ylabel('z_synapse', color='black')
    # ax2.set_ylabel("Amount(tag PRP y)")
    # # ax1.set_ylim(0.9, 1.55)
    ...
    # plt.title("synaptic weight changes : LTD")
    # plt.tight_layout(0.1)
    # plt.savefig('20Hz_10/2_prp100.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.
    #
    # # Visualization
    ...
    # plt.plot(t_tag_prp - 500, tag_prp_plt)
    # plt.xlim(0, 1000)
    # plt.xlabel("Time (ms)")
    # plt.ylabel('tag_flag')
    # plt.title("synapse_tag_flag")
    # plt.subplot(212)
    # plt.plot(t_tag_prp / time_scale, tag_prp_plt)
    # plt.xlabel("Time (min)")
    # plt.ylabel('tag_flag')
    # plt.xlim(0, 1)
    # plt.title("synapse tag_flag(1min)")
    # plt.tight_layout(0.1)
    # plt.savefig('20Hz_10/3_tag.jpg', bbox_inches='tight')
    # plt.close()
    # # plt.show()
    #
    #
    ...
    # for i in range(len(tag_flag_synapse)):
    #     if tag_flag_synapse[i] == 1 or tag_flag_synapse[i] == -1:
    #         tag_prp_plt[int(i * STC_1.tag_prp_check_window): int((i + 1) * STC_1.tag_prp_check_window)] = \
    #         tag_flag_synapse[i]
    # time_scale = 60 * 1000  ## min <-> ms
    ...
    # plt.title("synapse tag_flag(1min)")
    # plt.tight_layout(0.1)
    # plt.savefig('20Hz_10/4.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # # plt.show()
    #
    # soma_Ca_channel_average = np.zeros(5 * 60 * 60 * 1000)  ##5h  1ms为单位
    # dend2_Ca_channel_average = np.zeros(5 * 60 * 60 * 1000)
    # synapse_Ca_NMDA_average = np.zeros(5 * 60 * 60 * 1000)
    #
    # for i in range(len(soma_Ca_channel_average[0: repeat_interval*500])):
    #     if  i>=100:
    ...
    #         synapse_Ca_NMDA_average[i] = sum(synapse_Ca_NMDA[int(i/step -  STC_1.tag_prp_check_window / step):int(i/step)]) / int( STC_1.tag_prp_check_window / step)
    #     else:
    ...
    #
    #
    # t_average = np.arange(0, len(synapse_Ca_NMDA_average), 1)  ##5h  1ms为单位
    ...
    # ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window, synapse_Ca_NMDA[0:int(1500 / step)],label=' synapse Ca NMDA', color="r")
    # ax2.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window, dend2_Ca_channel[0:int(1500 / step)], '--',label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca NMDA (μmol/L)", color="r")
    # ax2.set_ylabel('dend Ca channel (μmol/L)', color="black")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('20Hz_10/9_1.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    #
    # fig = plt.figure(figsize = (6,5))
    # ax1 = fig.add_subplot(212)
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_average[0:1500] - 500, synapse_Ca_NMDA_average[0 + repeat_interval:1500 + repeat_interval], label=' synapse Ca NMDA',color="r")
    # lns3 = ax2.plot(t_average[0:1500] - 500, dend2_Ca_channel_average[0 + repeat_interval:1500 + repeat_interval], '--', label=' dend Ca channel(Threshold _PRP = 0.01)',color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.grid()
    # ax1.set_ylabel("synapse Ca NMDA(μmol/L)", color="r")
    ...
    #     if i >0 and V_s[i] >= V_s[i - 1]  and V_s[i] >= V_s[i+1] and V_s[i] >= -20:
    #         plt.axvline(x = (i*step-STC_1.calcium_time_window),ls = '--',c = 'y',linewidth = 1.0)
    # ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window, synapse_Ca_NMDA[0 + int(repeat_interval / step):int(1500 / step) + int(repeat_interval / step)],label=' synapse Ca NMDA', color="r")
    # ax2.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window, dend2_Ca_channel[0 + int(repeat_interval / step):int(1500 / step) + int(repeat_interval / step)], '--',label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca NMDA (μmol/L)", color="r")
    # ax2.set_ylabel('dend Ca channel (μmol/L)', color="black")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('20Hz_10/9_2.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize = (6,5))
    # ax1 = fig.add_subplot(212)
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_average[0:1500] - 500, synapse_Ca_NMDA_average[0 + repeat_interval*10:1500 + repeat_interval*10], label=' synapse Ca NMDA',color="r")
    # lns3 = ax2.plot(t_average[0:1500] - 500, dend2_Ca_channel_average[0 + repeat_interval*10:1500 + repeat_interval*10], '--', label=' dend Ca channel(Threshold _PRP = 0.01)',color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.grid()
    # ax1.set_ylabel("synapse Ca NMDA(μmol/L)", color="r")
    ...
    #     if i >0 and V_s[i] >= V_s[i - 1]  and V_s[i] >= V_s[i+1] and V_s[i] >= -20:
    #         plt.axvline(x = (i*step-STC_1.calcium_time_window),ls = '--',c = 'y',linewidth = 1.0)
    # ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window, synapse_Ca_NMDA[0 + int(repeat_interval*10 / step):int(1500 / step) + int(repeat_interval*10 / step)],label=' synapse Ca NMDA', color="r")
    # ax2.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window, dend2_Ca_channel[0 + int(repeat_interval*10 / step):int(1500 / step) + int(repeat_interval*10 / step)], '--',label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca NMDA (μmol/L)", color="r")
    # ax2.set_ylabel('dend Ca channel (μmol/L)', color="black")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('20Hz_10/9_10.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize = (6,5))
    # ax1 = fig.add_subplot(212)
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_average[0:1500] - 500, synapse_Ca_NMDA_average[0 + repeat_interval*100:1500 + repeat_interval*100], label=' synapse Ca NMDA',color="r")
    # lns3 = ax2.plot(t_average[0:1500] - 500, dend2_Ca_channel_average[0 + repeat_interval*100:1500 + repeat_interval*100], '--', label=' dend Ca channel(Threshold _PRP = 0.01)',color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.grid()
    # ax1.set_ylabel("synapse Ca NMDA(μmol/L)", color="r")
    ...
    #     if i >0 and V_s[i] >= V_s[i - 1]  and V_s[i] >= V_s[i+1] and V_s[i] >= -20:
    #         plt.axvline(x = (i*step-STC_1.calcium_time_window),ls = '--',c = 'y',linewidth = 1.0)
    # ax1.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window, synapse_Ca_NMDA[0 + int(repeat_interval*100 / step):int(1500 / step) + int(repeat_interval*100 / step)],label=' synapse Ca NMDA', color="r")
    # ax2.plot(t_neuron[0:int(1500 / step)] - STC_1.calcium_time_window, dend2_Ca_channel[0 + int(repeat_interval*100 / step):int(1500 / step) + int(repeat_interval*100 / step)], '--',label=' dend Ca channel', color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.set_ylabel("synapse Ca NMDA (μmol/L)", color="r")
    # ax2.set_ylabel('dend Ca channel (μmol/L)', color="black")
    ...
    # ax1.grid()
    # plt.tight_layout(0.1)
    # plt.savefig('20Hz_10/9_100.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    #
    #
    ...
    # ax1 = fig.add_subplot(211)
    # time_scale = 1000 * 60
    # plt.plot(time_1_hour / time_scale, firing_rate_list)
    # ax1.set_ylabel("firing rate (Hz)")
    # ax1.set_xlabel("time (min)")
    # plt.title("postneuron firing rate")
    # plt.tight_layout(0.1)
    # plt.savefig('20Hz_10/5.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(10.5, 7.5))
    # ax1 = fig.add_subplot(212)
    # plt.plot(t_neuron[0:int(1500/step)] - 500, V_s[0:int(1500/step)], label=' soma voltage', color="blue")
    ...
    # ax1.legend(loc=1)
    # plt.tight_layout(0.1)
    # plt.savefig('20Hz_10/6.jpg',bbox_inches = 'tight')
    # plt.close()
    # # plt.show()
    #
    # fig = plt.figure(figsize=(10.5, 7.5))
    ...
    # ax1.legend(loc=1)
    # plt.tight_layout(0.1)
    # plt.savefig('20Hz_10/6_1.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(10.5, 7.5))
    # ax1 = fig.add_subplot(212)
    # plt.plot(t_neuron[0 : int(1500/step)] - 500, V_s[0 + int(repeat_interval / step):int(1500/step) + int(repeat_interval / step)], label=' soma voltage', color="blue")
    # plt.plot(t_neuron[0 : int(1500/step)] - 500, V_d_1[0 + int(repeat_interval / step):int(1500/step) + int(repeat_interval / step)], "--", label=' dend1', color="red")
    # plt.plot(t_neuron[0 : int(1500/step)] - 500, V_d_2[0 + int(repeat_interval / step):int(1500/step) + int(repeat_interval / step)], "--", label=' dend2', color="black")
    # ax1.set_ylabel("Voltage (mV)")
    # ax1.set_xlabel("time (ms)")
    # plt.title("postneuron spike train ")
    # plt.xlim(0, 240)
    # ax1.legend(loc=1)
    # plt.tight_layout(0.1)
    # plt.savefig('20Hz_10/6_1_1.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(10.5, 7.5))
    # ax1 = fig.add_subplot(212)
    # plt.plot(t_neuron[0 : int(1500/step)] - 500, V_s[0 + int(repeat_interval *10 / step):int(1500/step) + int(repeat_interval *10 / step)], label=' soma voltage', color="blue")
    # plt.plot(t_neuron[0 : int(1500/step)] - 500, V_d_1[0 + int(repeat_interval *10 / step):int(1500/step) + int(repeat_interval *10 / step)], "--", label=' dend1', color="red")
    # plt.plot(t_neuron[0 : int(1500/step)] - 500, V_d_2[0 + int(repeat_interval *10 / step):int(1500/step) + int(repeat_interval *10 / step)], "--", label=' dend2', color="black")
    # ax1.set_ylabel("Voltage (mV)")
    # ax1.set_xlabel("time (ms)")
    # plt.title("postneuron spike train ")
    # plt.xlim(0, 240)
    # ax1.legend(loc=1)
    # plt.tight_layout(0.1)
    # plt.savefig('20Hz_10/6_1_10.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(10.5, 7.5))
    # ax1 = fig.add_subplot(212)
    # plt.plot(t_neuron[0 : int(1500/step)] - 500, V_s[0 + int(repeat_interval *100 / step):int(1500/step) + int(repeat_interval *100 / step)], label=' soma voltage', color="blue")
    # plt.plot(t_neuron[0 : int(1500/step)] - 500, V_d_1[0 + int(repeat_interval *100 / step):int(1500/step) + int(repeat_interval *100 / step)], "--", label=' dend1', color="red")
    # plt.plot(t_neuron[0 : int(1500/step)] - 500, V_d_2[0 + int(repeat_interval *100 / step):int(1500/step) + int(repeat_interval *100 / step)], "--", label=' dend2', color="black")
    # ax1.set_ylabel("Voltage (mV)")
    # ax1.set_xlabel("time (ms)")
    # plt.title("postneuron spike train ")
    # plt.xlim(0, 240)
    # ax1.legend(loc=1)
    # plt.tight_layout(0.1)
    # plt.savefig('20Hz_10/6_1_100.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    #
    # fig = plt.figure(figsize=(10.5, 7.5))
    # ax1 = fig.add_subplot(211)
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_neuron[0:int(1500/step)] - 500, synapse_Ca_NMDA[0:int(1500/step)], label=' synapse_Ca_NMDA',color="r")
    # lns3 = ax2.plot(t_neuron[0:int(1500/step)] - 500, dend2_Ca_channel[0:int(1500/step)], '--', label=' dend_Ca_channel',color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.grid()
    # ax1.set_ylabel("synapse_Ca_NMDA (μmol/L)", color="r")
    # ax2.set_ylabel('dend_Ca_channel (μmol/L)', color="black")
    # lns = lns1 + lns3
    # labs = [l.get_label() for l in lns]
    # ax1.legend(lns, labs, loc=1)
    # plt.title("changes in calcium concentration")
    # plt.tight_layout(0.1)
    # plt.savefig('20Hz_10/7.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(10.5, 7.5))
    # ax1 = fig.add_subplot(211)
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_neuron[0:int(1500/step)] - 500, synapse_Ca_NMDA[0 + int(repeat_interval/step):int(1500/step) + int(repeat_interval/step)], label=' synapse_Ca_NMDA',color="r")
    # lns3 = ax2.plot(t_neuron[0:int(1500/step)] - 500, dend2_Ca_channel[0 + int(repeat_interval/step):int(1500/step) + int(repeat_interval/step)], '--', label=' dend_Ca_channel',color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.grid()
    # ax1.set_ylabel("synapse_Ca_NMDA (μmol/L)", color="r")
    ...
    # plt.title("changes in calcium concentration")
    # plt.tight_layout(0.1)
    # plt.savefig('20Hz_10/7_1.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # fig = plt.figure(figsize=(10.5, 7.5))
    # ax1 = fig.add_subplot(211)
    # ax2 = ax1.twinx()
    # lns1 = ax1.plot(t_neuron[0:int(1500/step)] - 500, synapse_Ca_NMDA[0 + int(repeat_interval*10/step):int(1500/step) + int(repeat_interval*10/step)], label=' synapse_Ca_NMDA',color="r")
    # lns3 = ax2.plot(t_neuron[0:int(1500/step)] - 500, dend2_Ca_channel[0 + int(repeat_interval*10/step):int(1500/step) + int(repeat_interval*10/step)], '--', label=' dend_Ca_channel',color="black")
    # plt.xlim(0, 1000)
    # ax1.set_xlabel("time (ms)")
    # ax1.grid()
    # ax1.set_ylabel("synapse_Ca_NMDA (μmol/L)", color="r")
    ...
    # plt.title("changes in calcium concentration")
    # plt.tight_layout(0.1)
    # plt.savefig('20Hz_10/7_9.jpg',bbox_inches = 'tight')
    # plt.close()
    #
    # # fig = plt.figure(figsize=(10, 5))
    # # bax = brokenaxes(xlims=((-2, 50), (250, 302)),hspace=.05, despine=True)
    # # lns1 = bax.plot(t_neuron/(60*1000), synapse_Ca_NMDA, label=' synapse Ca NMDA',color="r")  ## 没有滑动平均的结果
    ...
    # # bax.legend(loc=1)
    # # plt.title("changes in calcium concentration")
    # # plt.savefig('20Hz_10/7_1_1.jpg',bbox_inches = 'tight')
    # # plt.close()
    #
    # #
    # # fig = plt.figure(figsize=(10.5, 7.5))
    ...
    # # lns1 = ax1.plot(t_neuron_sub / time_scale_sub, synapse_Ca_NMDA_sub, label=' synapse_Ca_NMDA',color="r")
    # # lns3 = ax1.plot(t_neuron_sub / time_scale_sub, dend2_Ca_channel_sub, '--', label=' dend_Ca_channel',color="black")
    # # ax1.set_xlabel("time (min)")
    # # ax1.set_ylabel(" Ca concentration (μmol/L)")
    # # ax1.grid()
    # # lns = lns1 + lns3
    # # labs = [l.get_label() for l in lns]
    # # ax1.legend(lns, labs, loc=1)
    # # plt.title("changes in calcium concentration")
    # # plt.tight_layout(0.1)
    # # plt.savefig('20Hz_10/8.jpg')
    # # # plt.show()
    # #
    # # fig = plt.figure(figsize=(10, 5))
    # # bax = brokenaxes(xlims=((-2, 50), (250, 302)),hspace=.05, despine=True)
    # # time_scale_sub = (1000 / STC_1.tag_prp_check_window) * 60
    # # bax.plot(t_neuron_sub / time_scale_sub, synapse_Ca_NMDA_sub, label=' synapse Ca NMDA',color="r")  ## 做了滑动平均的结果
